...
# Как оформляется фактура за уже удержанную онлайн-комиссию, когда сверху ложится
# налог. Пусто = не решено, и такие фактуры не выпускаются вовсе (см. _issue_to_stripe).
_ONLINE_FEE_TAX_MODEL = (os.getenv("BILLING_ONLINE_FEE_TAX_MODEL") or "").strip().lower()


class OnlineFeeTaxModelUndecided(RuntimeError):
    """Фактура за удержанную комиссию разошлась бы с реально полученными деньгами.

    Не сбой Stripe и не долг студии: до решения владельца документ не выпускается,
    а начисления остаются неотфактурованными и попадут в следующий проход.
    """
...
async def _issue_to_stripe(
    ...
    * `online_fee` — деньги уже удержаны Stripe в момент платежа клиента, поэтому
    ...
    if invoice.kind == "online_fee":
        # ОТДЕЛЬНОЕ ФИНАНСОВОЕ РЕШЕНИЕ, а не ошибка кода — и до его принятия документ
        # НЕ ВЫПУСКАЕТСЯ. Свою долю Stripe удержал в момент платежа клиента: это
        # НЕТТО. Если сверху начисляется налог, документ объявляет полученной сумму
        # БОЛЬШЕ удержанной, а `paid_out_of_band` утверждает «эти деньги уже у нас».
        # Про налоговую часть это неправда, и такая фактура расходится с выпиской.
        #
        # Чинить догадкой нельзя ни в одну сторону: уменьшить комиссию — значит молча
        # изменить цену услуги; удержать больше — значит списать со студии деньги, о
        # которых с ней не договаривались. Поэтому решение приходит конфигурацией и
        # должно быть подтверждено владельцем или бухгалтером:
        #
        #   net_withheld_gross_invoiced — удержано нетто, фактура выставляется на
        #       брутто. Налог платформа платит из своих: получила X, задекларировала
        #       X + НДС. Юридически возможно, экономически — минус НДС с каждой
        #       комиссии;
        #   gross_withheld — удержанное считается суммой С НАЛОГОМ. Документ сходится
        #       с выпиской до цента, но эффективная ставка комиссии для студии
        #       меняется, и это изменение условий договора.
        #
        # Пока не выбрано — стоп. Выпустить документ, не сходящийся с деньгами,
        # хуже, чем не выпустить его вовсе: он попадёт в учёт обеих сторон.
        if snapshot.get("tax_amount") and not _ONLINE_FEE_TAX_MODEL:
            raise OnlineFeeTaxModelUndecided(
                f"студия {invoice.studio_id}, период {invoice.period}: удержано "
                f"{invoice.amount} {stripe_billing.CURRENCY} (нетто), налог "
                f"{snapshot['tax_amount']}, документ объявил бы оплаченными "
                f"{invoice.amount + snapshot['tax_amount']}. Расхождение нетто/брутто "
                f"не решено — задайте BILLING_ONLINE_FEE_TAX_MODEL"
            )
