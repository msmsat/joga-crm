"""Лимиты тарифа (задача 8): не дать превысить каталожный потолок сотрудников.

Источник цифр — каталог plans.py (задача 2). Business (None) = безлимит.
Клиентов тариф больше не ограничивает вовсе — потолок снят на всех ступенях
(plans._limits), и сообщение «улучшите тариф» осталось только про места.
...
#
# Клиенты остаются в таблице, хотя потолка у них больше нет ни на одной ступени
# (plans._limits: "clients" всегда None). Вызов на создании клиента сохранён
# намеренно: он же проверяет истёкшую подписку — при выключенном глобальном
# гейте это единственная преграда. И если ограничение когда-нибудь вернут, оно
# вернётся правкой каталога, а не восстановлением механизма здесь.
_ENTITIES = {
