...
class PersonalKind(str, Enum):
    ...
    # серверную проверку.
    VERIFY_ME = "verify_me"
    # «Запиши меня», «беру», «давай второй» — просьба ЗАПИСАТЬСЯ на занятие,
    # которое сервер уже показал. Какое именно — модель не говорит и не может:
    # занятие определяется непрозрачной ссылкой из показанного списка либо
    # порядковым номером (`selection`), и то и другое разрешает сервер.
    #
    # Само по себе это значение ничего не бронирует. Оно приводит к
    # ПРЕДЛОЖЕНИЮ (services/proposals), у которого нет побочных эффектов, и
    # только явное согласие человека запускает переход.
    BOOK_LESSON = "book_lesson"
    # «Да», «подтверждаю», «записывай» — согласие на уже показанное
    # предложение. Если живых предложений не одно, сервер НЕ угадывает.
    CONFIRM = "confirm"
