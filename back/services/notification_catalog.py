...
# от матрицы и глобальных тумблеров). Telegram/Instagram — диалоговые каналы:
# писать в них можно только тому, кто сам начал диалог (клиент — через
# мини-приложение при записи, Bot API не даёт написать первым). У сотрудника
# такого пути нет — открывать клиентское мини-приложение тренеру или
# администратору незачем, а вручную вписанный chat_id не делает канал
# осмысленным каналом уведомлений. WhatsApp телефон — не диалог, а обычный
# реквизит из профиля, поэтому доступен и персоналу наравне с email.
