...
def expects_livemode() -> bool | None:
    """Каким режимом работает ключ платформы: True — боевой, False — тестовый,
    None — ключа нет или он неопознан.

    Нужно ровно одному месту — вебхуку оплат: событие тестового режима не имеет
    права закрывать боевую заявку и наоборот. Подпись такое событие обычно и не
    пропустит (секреты у режимов разные), но «обычно» про деньги не аргумент.
    """
    mode = key_mode(_secret())
    return None if mode == MODE_UNKNOWN else mode == MODE_LIVE
