...
def _gave_up(text: str | None) -> bool:
    """Первый ход, ни одного инструмента — это отказ работать или простое «спасибо»?

    Раньше приметой был только знак вопроса. Отписка обошлась без него ни разу
    («Без этих данных создать сотрудников я не смогу.»), эскалация не сработала,
    и человек получил допрос вместо окна подтверждения — при том, что все эти
    поля собирает форма, а не он.

    Длина и знак вопроса, а не список слов: студий с английским языком столько
    же, сколько с русским, и «I'll need their emails and a password» ни одно
    русское слово не поймает. Ошибиться эта примета может только в одну
    сторону — лишний раз спросить дорогую голову, а не отдать человеку отписку.
    """
    text = (text or "").strip()
    return bool(text) and ("?" in text or len(text) >= _GAVE_UP_LEN)
