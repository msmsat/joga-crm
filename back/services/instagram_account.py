...
подключается по OAuth на странице AI) и StudioIntegration/ig_dm (карточка в
Настройках → Интеграции). Раньше каждая страница писала только в свою:
подключённый в AI инстаграм не появлялся во второй, там его приходилось
...
Direct — канал ТОЛЬКО реактивный: ИИ-агент отвечает на входящие в 24-часовом окне
(routers/ai/instagram.py). Проактивных рассылок здесь нет и быть не может — в
Уведомлениях Instagram отключён именно поэтому (см. докстринг services/notifier.py).
...
токен, введённый руками в Настройках → Интеграции («…with Facebook Login») — на
graph.facebook.com. Читателей у ключа сейчас нет (агент ходит на graph.instagram.com
напрямую) — он остался пометкой о происхождении токена; у старых строк ключа нет,
это facebook_login, как и было до появления OAuth.
"""
