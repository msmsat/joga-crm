...
def migrate_existing_allowed() -> bool:
    """Разрешено ли ФОНОВОЙ задаче переводить УЖЕ СУЩЕСТВУЮЩИЕ подписки.

    Отдельный флаг от `BILLING_TAX_MODE`, и это не перестраховка. Без него
    переключение режима означало бы, что ежечасный проход молча пойдёт править
    живые подписки — то есть production-миграция случилась бы как побочный эффект
    правки переменной окружения, без dry-run, без манифеста и без согласования.

    Порядок задуман такой: сначала режим (новые документы идут по ручным ставкам),
    и только потом, отдельным осознанным шагом, — существующие объекты. Между этими
    шагами система работает корректно: старая подписка продолжает считать по-старому.
    """
    return (os.getenv("BILLING_TAX_MIGRATE_EXISTING") or "").strip().lower() in ("1", "true", "yes")
