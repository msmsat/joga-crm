...
_PRICES: dict[str, tuple[int, int, int, int]] = {
    ...
    # Рабочий уровень FAST. Запись кэша тут ДЕШЕВЛЕ обычного входа (0.0417 против
    # 0.75) — редкий случай, и в таблице она однажды стояла вшестеро дороже правды:
    # 125_000 было переписано по образцу соседей, а не сверено запросом.
    # Провайдер поднял все четыре графы ровно вдвое — сверено живым /v1/models
    # 14.09.2026 (поймал preflight). До правки ai_usage занижал расход FAST вдвое.
    "google/gemini-3.7-flash":      (750_000,   75_000,    41_667,  3_750_000),
