...
#
# ЭТО СРОК ЖИЗНИ РАЗГОВОРА, А НЕ ГАРАНТИЯ ЗАПИСИ. Прежняя формулировка выводила
# час из окна записи (`BookingRules.min_booking_advance_min`, по умолчанию 120
# минут) — и была неверна: вариант, показанный за 121 минуту до начала, через
# час оказывается уже ВНУТРИ закрытого окна. Правильность от этого не страдала
# ни дня — P1 только читает, выбранное занятие перечитывается из каталога, а
# запись, когда она появится, обязана проверить правила заново, — но объяснять
# срок несуществующей гарантией нельзя: на такое объяснение однажды сошлются.
#
# Час — это про человека: столько живёт «второй вариант» в мессенджере.
# Позже расписание и места успевают измениться настолько, что честнее искать
