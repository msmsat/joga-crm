...
def test_future_assignment_includes_after_buffer(monkeypatch):
    monkeypatch.setattr(schedule_guard.lesson_time, 'local_now', lambda studio: datetime(2027, 6, 15))
    async def run():
        ids = await rh._seed()
        try:
            await rh._assign(ids)
            ...
            async with async_session_maker() as db:
                ...
                db.add(lesson)
                busy = StaffBusyInterval(studio_id=ids['studio'], user_id=ids['teacher'],
                ...
                await db.delete(busy)
                await db.flush()
                assert await schedule_guard.assert_future_assignments_valid(db, studio, user_id=ids['teacher']) == []
