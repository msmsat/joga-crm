...
# ── Лестница длины ответа доезжает до модели ──────────────────────────────────
#
# Ассистент по умолчанию отвечал отчётом: на «сколько записей сегодня» выдавал
# разбивку по тренерам, после собранного действия пересказывал поля, которые
# человек и так видит в карточке подтверждения. Лечится это только промптом —
# длину ответа за модель никто не обрежет, — поэтому правило пиннится здесь.
# Само поведение проверяет живой набор (scripts/ai_eval), тут — бесплатный
# рубеж: правило не должно пропасть при следующей переписке промпта.

def test_answer_length_ladder_is_part_of_the_rules():
    rules = assistant._RULES
    # Якоря по 80-колоночной вёрстке: словосочетание через перенос строки
    # тест бы не нашёл.
    for must in ("ДЛИНА ОТВЕТА", "1-3 строки", "одна строка, и всё",
                 "Не дописывай хвост", "от четырёх однотипных пунктов"):
        assert must in rules, must


def test_length_rule_reaches_the_uncached_reminder():
    # Слот [3] — единственный, который читается вплотную к вопросу человека.
    # Правило про длину нарушалось чаще прочих, поэтому оно живёт и здесь,
    # хотя этот слот не кэшируется и оплачивается каждым запросом.
    assert "1-3 строки" in assistant._REMINDER
