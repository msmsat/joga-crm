...
UI_ONLY: dict[str, str] = {
    ...
    "POST /presence/beat": "сигнал «вкладка открыта» от анонимного браузера: "
                           "ничего не меняет в базе, только отметка в памяти",
    ...
    "DELETE /auth/me": "удаление собственного аккаунта вместе с почтой — необратимое "
                       "решение человека о себе; подтверждается кодом с почты, и "
                       "инструмент ассистента здесь означал бы, что аккаунт можно "
                       "снести разговором",
    ...
    "POST /studio/upload-note-photo": "загрузка файла",
