...
# Скидка за период оплаты: 3 мес −20%, 6 мес −25%, 12 мес −30%.
PERIOD_DISCOUNTS: dict[int, float] = {1: 0.0, 3: 0.20, 6: 0.25, 12: 0.30}
...
# тарифов у нас все в одном файле. Эта же цифра стоит на лендинге и в окне
# акции — `tests/test_trial_days_sync.py` не даёт им разойтись.
TRIAL_DAYS = 30
...
def break_even_turnover(plan_id: str, mode: str = "percent") -> int:
    ...
    (30 €/мес) порог — 1000 € оборота в месяц, у безлимита (150 €) — 5000 €.
    Студия, у которой за месяц прошло 5000 €, на подписке заплатит 30 €, на
    «проценте» — 150 €, на «комбо» — 90 €.
...
def combo_amount_for(plan_id: str, period_months: int) -> int:
    ...
    Считается ОТ ИТОГА подписки, а не от половинной месячной цены: как только
    итог со скидкой периода выходит нечётным в центах, две половинки по своей
    формуле дают в сумме на цент меньше — и «комбо стоит ровно половину»
    перестаёт быть правдой. Лишний цент делим в пользу студии (floor).

    На текущем каталоге нечётных итогов нет (все цены кратны 5 €, все скидки —
    целые проценты), но зависеть от этого нельзя: одна новая ступень или ставка
    возвращает случай, а самотест ниже держит равенство на всём каталоге.
    """
