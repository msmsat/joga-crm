...
async def sync_subscription_taxes(db: AsyncSession) -> int:
    ...
    ВАЖНО: сама по себе смена `BILLING_TAX_MODE` этот проход НЕ включает. Правка
    живых подписок — это production-миграция, и случаться она должна отдельным
    осознанным шагом (`BILLING_TAX_MIGRATE_EXISTING=1` либо
    `scripts/migrate_tax_mode --apply`), а не как побочный эффект переменной
    окружения. Иначе переключение режима на проде тихо переписало бы чужие
    подписки без dry-run и без согласования.
    """
    if not tax_policy.manual_mode():
        return 0
    if not tax_policy.migrate_existing_allowed():
        return 0
