...
class AIQuotaRead(BaseSchema):
    """Остаток квоты ИИ (эпик AI-5, задача 3). Денежный потолок сюда
    не попадает — это себестоимость платформы, а не обещание студии."""
    used: int
    limit: int
    # Пробный потолок на всё время вместо месячного тарифного: от него UI меняет
    # текст и убирает «Перейти на новый тариф» — апгрейд его не поднимает.
    trial: bool = False
