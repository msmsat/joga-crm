...
class IbanCheckoutResponse(BaseModel):
    ...
    # Держателя счёта называет Stripe (счёт коллекторский, держит его он), и это
    # значение подставляет checkout.create_iban_checkout. Дефолт — ПУСТАЯ строка, а не
    # имя: раньше здесь стояло «Velora CRM LLC», юрлицо, которого не существует —
    # продавец по Условиям это Andrii Sadovskyi, OSVČ. Студия вписала бы это имя в
    # платёжное поручение, банк не свёл бы получателя (Verification of Payee в ЕС
    # обязательна) и предупредил бы или отбил перевод, а в её учёте появился бы
    # несуществующий контрагент. Пусто фронт просто не показывает — это честнее
    # неверного имени.
    beneficiary: str = ""
