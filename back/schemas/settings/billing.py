...
class PlansCatalogRead(BaseSchema):
    ...
    min_monthly: int
    # Условия постоплаты — теми же цифрами, какими они применяются на сервере.
    # Их называет модалка согласия, и владелец подтверждает ИМЕННО ИХ: ставку, с
    # которой начисляется комиссия, и срок, после которого доступ блокируется.
    # Пока они стояли литералами в разметке, правка PERCENT_ONLY_RATE или
    # GRACE_DAYS оставляла согласие обещающим прежние 3 % и прежнюю неделю —
    # то есть документ и поведение расходились молча.
    percent_rate: float                  # тариф «только процент», %
    combo_rate: float                    # тариф «фикс + процент», %
    grace_days: int                      # дней на оплату счёта до блокировки
...
class BillingPlanRead(BaseSchema):
    ...
    # Открыта ли ещё акция «N дней бесплатно». Считает сервер по тому же правилу,
...
class BillingProfileRead(BaseSchema):
    ...
    # «реестр в момент ввода молчал»: номер сохранён, но в Stripe не уезжает, и
    # плательщику выставляется полный НДС. Форма реквизитов обязана сказать об
    # этом вслух — иначе счёт с налогом выглядит ошибкой платформы.
...
class CheckoutPreviewRead(BaseSchema):
    ...
    # (services/billing_tax.preview). Своей арифметики налога на фронте нет и быть
    # не должно: разошедшиеся цифры в модалке и в счёте — это спор с клиентом о
    # сумме, а не косметика. В Stripe за расчётом при этом не ходим ни в каком
    # режиме — платный вызов ради превью не оправдан ничем.
    #
    # taxable — налог начисляется; reverse_charge — платит покупатель; exempt —
    # освобождение; out_of_scope — вне европейского НДС; requires_review — решения
    # нет, и сумму нельзя показывать как окончательную; stripe_auto — прежний режим,
    # ставку определит страница Stripe.
    tax_outcome: str = "stripe_auto"
    tax_rate_percent: float = 0.0
    tax_amount: int = 0
    # Итог с налогом. Равен total, когда налога нет; в режиме stripe_auto тоже равен
    # total — там сумма налога станет известна только на странице Stripe.
    total_with_tax: int = 0
    # Заполнено только при requires_review: короткая причина для интерфейса, чтобы
    # владелец понимал, чего не хватает, и не считал это отказом банка.
