...
TERMS_VERSION поднимать РУКАМИ при каждой правке static/terms.html,
static/privacy.html или static/cookies.html и ставить ту же дату в шапку всех
трёх файлов. Версия — это то, чем доказывается «человек согласился именно с этой
редакцией»; без её подъёма старые согласия молча начнут выглядеть как согласия
с новым текстом.

Согласие на cookie сюда НЕ пишется: оно живёт в браузере посетителя
(front/src/utils/cookieConsent.ts) — до входа человека у нас нет аккаунта, к
которому его можно было бы привязать.
"""
import os
...
COOKIES_URL = f"{BACKEND_URL}/static/cookies.html"
