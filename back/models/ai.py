...
class AIUsage(Base):
    ...
    request_id: Mapped[Optional[str]] = mapped_column(String(32), nullable=True, index=True)
    # Язык, на котором ответили, и почему именно он. Не украшение телеметрии:
    # в мессенджерах агент вызывается по ОДНОМУ сообщению, истории у него нет,
    # и «ОК» после чешского вопроса иначе падало бы на язык студии. Здесь
    # хранится последний надёжный язык РАЗГОВОРА — выборка идёт по той же тройке
    # studio_id + surface + sender_ref, что и антиспам, поэтому языки двух
    # клиентов одной студии не смешиваются.
    # Заодно отвечает на боевой вопрос «почему ассистент ответил по-чешски»
    # без чтения мыслей: рядом лежит источник решения.
    response_language: Mapped[Optional[str]] = mapped_column(String(8), nullable=True)
    language_source: Mapped[Optional[str]] = mapped_column(String(24), nullable=True)
