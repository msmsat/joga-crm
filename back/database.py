...
#
# Размер пула задаём явно. Дефолт SQLAlchemy — 5 постоянных плюс 10 запасных,
# то есть 15 одновременных запросов к БД на процесс; шестнадцатый ждёт
# `pool_timeout` (30 с) и падает. Обычным ручкам этого хватало бы с запасом —
# они держат соединение миллисекунды. Упирается в потолок SSE-поток ассистента
# (routers/ai/chat.py): он открывает свою сессию и держит её ВЕСЬ разговор,
# 5–15 секунд, пока модель зовёт инструменты. Пятнадцать одновременных чатов —
# и пул пуст для всех остальных страниц студии.
#
# Потолок соединений у Postgres общий на всех, кто к нему ходит: api и worker —
# одинаковые процессы с этим же движком, 40 + 40 против дефолтных
# max_connections = 100. Поднимать пул дальше — только вместе с max_connections,
# иначе вместо ожидания в пуле получим отказ уже от самой базы.
_POOL_KWARGS = (
    {"poolclass": NullPool} if "pytest" in sys.modules
    else {"pool_size": 20, "max_overflow": 20}
)
