...
**Приём настоящих денег — отдельный чеклист: [docs/GO-LIVE.md](docs/GO-LIVE.md).**
...
## Панель платформы (кабинет владельца продукта)

Отдельный сайт со своим логином: трафик лендинга, регистрации, входы, пробные
периоды и деньги. Собирается на хосте, ровно как мини-приложение:
...
папки — нет панели, API при этом работает. Пересобирать образ после правок не
нужно, достаточно `npm run build`.

Аккаунт задаётся в `back/.env`. **Не заданы все три обязательные переменные —
панель выключена целиком**, а не открыта нараспашку:
...
выданный кабинетом токен был бы валидной подписью и для панели. Расхождение
...
Формы смены пароля в интерфейсе нет намеренно — нет формы, нечего и ломать.

**Перед первым выкатом сделать бэкап:** миграция `aa457ff31dab` добавляет колонки
в `users` и `studios` и заполняет их по истории входов. Она не разрушающая, но
трогает две центральные таблицы — `./backup.sh` перед `docker compose up`.
...
окружения отдельного сервиса команда с `--force-recreate api` приведена ниже.

Сессии пользователей переживают пересборку: токен хранится в браузере, ключ
`SECRET_KEY` берётся из постоянного `back/.env`, записи сессий — из базы в томе
`pgdata`. Не генерируй новый `SECRET_KEY` при деплое: его смена делает ранее
выданные токены недействительными. При временной ошибке сети, таймауте или 5xx
проверка входа сохраняет токен и повторяется автоматически. Проверка регрессий:
`cd front && npm run check:session`. Уже удалённый старой версией токен
