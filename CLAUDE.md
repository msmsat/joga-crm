...
5. Меняешь страницу (новая кнопка, переименованная вкладка, другой путь) → правишь `back/services/ai_uimap.md`. По этой карте ассистент объясняет пользователю, куда нажать; устаревшая карта не ломает сборку — она заставляет ИИ уверенно называть кнопки, которых больше нет.
