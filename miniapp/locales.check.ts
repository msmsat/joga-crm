/**
 * Каждый ключ, который просит интерфейс, должен быть во ВСЕХ пяти языках.
 *
 *   cd miniapp && node locales.check.ts
 *
 * Ловит ровно то, на чём тут уже спотыкались: ключ добавили в ru и забыли в
 * cs/uk/en (клиент видит «subscriptionSheet.buy» вместо кнопки), либо строку
 ...
 *
 * Список языков держать в паре с `src/locales/`: чешский лежит в `cs.json`
 * (тег локали), а не в `cz.json` (страна), и немецкого в проверке не было
 * вовсе — из-за этого она падала на чтении файла и не проверяла ничего.
 *
...
const LOCALES = ['ru', 'en', 'uk', 'cs', 'de'] as const;
