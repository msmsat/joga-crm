...
export interface AIChatMessage {
  ...
  rating?: number | null
  // Есть ли у карточки кнопка «Вернуть»: действие создало что-то, что
  // откатывается честно. Флаг, а не список id — что именно сносить, решает
  // сервер по своей записи, принимать это от браузера он не станет.
  can_undo?: boolean
}
...
export interface AIPlanStep {
  n: number
  tool: string
  // Короткое имя действия без значений («Завести сотрудника»): по нему чат
  // сворачивает подряд идущие одинаковые шаги в одну строку со счётчиком.
  title: string
