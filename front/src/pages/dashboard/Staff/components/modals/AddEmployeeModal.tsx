      ...
      {roleChips.map((chip, i) => {
        ...
        return (
          <g key={i} opacity={pos.opacity}>
            <rect x={pos.x} y={pos.y} width="64" height="26" rx="13" fill="rgba(26,26,26,0.04)" stroke="#EEEBE6" strokeWidth="1"/>
        ...
        <div style={{ padding: "16px 30px 24px", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          <div className="as-scroll" style={{ flex: 1, overflowY: "auto", paddingRight: "4px", marginTop: "28px" }}>
            <div key={step} style={{ animation: "stepIn 0.25s cubic-bezier(0.34,1.1,0.64,1)" }}>
              ...
              {step === 1 && (
                <div>
                  <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.6px", margin: "0 0 4px" }}>{t("addModal.step1.heading")}</h3>
                    ...
                    {data.name.trim().length >= 2 && (
                      <div style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: "11px", color: "#AAA", marginTop: "2px" }}>{data.phone || data.email || t("addModal.step1.contactsNotSet")}</div>
                        </div>
