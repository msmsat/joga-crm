...
interface StaffData {
  ...
  role: string; services: string[];
  salary: string; salaryType: "fixed" | "percent" | "hourly" | "";
  ...
  return (
    <svg viewBox="0 0 260 240" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", maxWidth: 260 }}>
      <defs>
        ...
        <filter id="i2shadow"><feDropShadow dx="0" dy="6" stdDeviation="12" floodColor="rgba(26,26,26,0.08)"/></filter>
...
function Illus3({ schedule }: { schedule: Record<string, ScheduleDay> }) {
  ...
  return (
    <svg viewBox="0 0 260 240" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", maxWidth: 260 }}>
      <defs>
        <filter id="i3shadow"><feDropShadow dx="0" dy="8" stdDeviation="16" floodColor="rgba(26,26,26,0.09)"/></filter>
...
function Logo() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "9px" }}>
      ...
      <span style={{ fontSize: "15px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.4px", fontFamily: "Manrope, sans-serif" }}>
        velora
      </span>
    </div>
  );
...
export function AddEmployeeModal({ isOpen, onClose, onSuccess }: AddEmployeeModalProps) {
  ...
  function handleFinish() { onSuccess?.(data); handleClose(); }
    ...
    >
      ...
      <div
        style={{
          ...
          overflow: "hidden", animation: "modalIn 0.42s cubic-bezier(0.34,1.1,0.64,1)",
        }}
          ...
          <div style={{
            ...
            {step < 4 ? (
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <span style={{ fontSize: "11px", color: "#666", fontWeight: 500 }}>{current.trustLabel}</span>
              </div>
            ) : (
        ...
        <div style={{ padding: "16px 30px 24px", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          <div className="as-scroll" style={{ flex: 1, overflowY: "auto", paddingRight: "4px", marginTop: "28px" }}>
            <div key={step} style={{ animation: "stepIn 0.25s cubic-bezier(0.34,1.1,0.64,1)" }}>
              ...
              {step === 1 && (
                <div>
                  <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.6px", margin: "0 0 4px" }}>Личные данные</h3>
                  <p style={{ fontSize: "12px", color: "#AAA", margin: "0 0 24px" }}>Начнём с имени и способа связи</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                    <div>
                      <FieldLabel>Полное имя *</FieldLabel>
                      <FocusInput value={data.name} onChange={v => set("name", v)} placeholder="Например: Анна Новикова"/>
                    </div>
                    <div>
                    ...
                    {data.name.trim().length >= 2 && (
                      <div style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: "13px", fontWeight: 700, color: "#1A1A1A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{data.name}</div>
                          <div style={{ fontSize: "11px", color: "#AAA", marginTop: "2px" }}>{data.phone || data.email || "Контакты не указаны"}</div>
                        </div>
                        <div style={{ padding: "3px 8px", borderRadius: "6px", background: "rgba(163,201,168,0.18)", fontSize: "9px", fontWeight: 700, color: "#5A8A60", letterSpacing: "0.5px" }}>НОВЫЙ</div>
                      </div>
                    )}
              ...
              {step === 2 && (
                <div>
                  <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.6px", margin: "0 0 4px" }}>Роль и услуги</h3>
                  <p style={{ fontSize: "12px", color: "#AAA", margin: "0 0 20px" }}>Выберите должность — услуги подберутся автоматически</p>
                  ...
                  <div>
                    ...
                    <div style={{ position: "relative" }}>
                      <FocusInput value={customServiceInput} onChange={setCustomServiceInput} onKeyDown={handleServiceKeyDown} placeholder="Добавить услугу — нажмите Enter"/>
                      {customServiceInput && (
              ...
              {step === 3 && (
                <div>
                  <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.6px", margin: "0 0 4px" }}>График и зарплата</h3>
                  <p style={{ fontSize: "12px", color: "#AAA", margin: "0 0 20px" }}>Настройте рабочие дни и вознаграждение</p>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", gap: "7px", marginBottom: "10px" }}>
                      {(["fixed", "percent", "hourly"] as const).map(type => {
                        ...
                        return (
                          <button key={type} type="button" onClick={() => set("salaryType", type)} style={{
              ...
              {step === 4 && (
                <div>
                  <h3 style={{ fontSize: "20px", fontWeight: 900, color: "#1A1A1A", letterSpacing: "-0.6px", margin: "0 0 4px" }}>
                    Отлично!{" "}<span style={{ color: "#FCAE91" }}>{data.name.split(" ")[0] || "Сотрудник"}</span> добавлен
                  </h3>
                  <p style={{ fontSize: "12px", color: "#AAA", margin: "0 0 20px", lineHeight: 1.6 }}>Теперь пригласите сотрудника присоединиться к системе</p>
                  <div style={{ padding: "16px 18px", background: "rgba(26,26,26,0.025)", borderRadius: "14px", border: "1.5px solid rgba(26,26,26,0.08)", marginBottom: "14px" }}>
                    <p style={{ fontSize: "10px", fontWeight: 700, color: "#AAA", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 7px" }}>Ссылка-приглашение</p>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <p style={{ fontSize: "10px", fontWeight: 700, color: "#AAAAAA", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 9px" }}>Отправить приглашение</p>
                  <div style={{ display: "flex", gap: "8px", marginBottom: "14px" }}>
                    <button type="button" disabled={!data.phone} style={{ flex: 1, padding: "12px 14px", background: data.phone ? "rgba(37,211,102,0.08)" : "rgba(26,26,26,0.03)", border: data.phone ? "1.5px solid rgba(37,211,102,0.3)" : "1.5px solid rgba(26,26,26,0.07)", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", gap: "7px", cursor: data.phone ? "pointer" : "default", fontFamily: "Manrope, sans-serif", fontSize: "13px", fontWeight: 600, color: data.phone ? "#25d366" : "#CCC", transition: "all 0.15s" }}>
                      ...
                      WhatsApp{!data.phone && <span style={{ fontSize: "10px", opacity: 0.6 }}>(нет номера)</span>}
                    </button>
                    <button type="button" disabled={!data.email} style={{ flex: 1, padding: "12px 14px", background: data.email ? "rgba(252,174,145,0.08)" : "rgba(26,26,26,0.03)", border: data.email ? "1.5px solid rgba(252,174,145,0.3)" : "1.5px solid rgba(26,26,26,0.07)", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", gap: "7px", cursor: data.email ? "pointer" : "default", fontFamily: "Manrope, sans-serif", fontSize: "13px", fontWeight: 600, color: data.email ? "#F9A08B" : "#CCC", transition: "all 0.15s" }}>
                      ...
                      Email{!data.email && <span style={{ fontSize: "10px", opacity: 0.6 }}>(нет почты)</span>}
                    </button>
                  </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginTop: "20px", paddingTop: "16px", borderTop: "1px solid #F0EDE8", flexShrink: 0 }}>
            {step > 1 && step < 4 && (
              <button type="button" className="as-back-btn" onClick={goBack} style={{
                ...
                borderRadius: "12px", fontSize: "13px", fontWeight: 600, color: "#888",
