...
import { SALES_DATA, PERIOD_LABELS } from '../../constants';
...
const DAILY_CHECKS = [1800, 2200, 1950, 2400, 2100, 1700, 2800];
    ...
    <>
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        ...
        {SALES_DATA.map((row, i) => {
          const barPct = Math.round((parseInt(row.revenue.replace(/[₽K]/g,'')) / parseInt(SALES_DATA[0].revenue.replace(/[₽K]/g,''))) * 100);
          return (
            <div key={i} onClick={() => setSelected(selected === i ? null : i)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '10px', cursor: 'pointer', marginBottom: '4px', background: selected === i ? 'rgba(252,174,145,0.08)' : 'transparent', border: `1px solid ${selected === i ? 'rgba(252,174,145,0.3)' : 'transparent'}`, transition: 'all 0.18s' }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '6px', height: '100px' }}>
            {DAILY_CHECKS.map((v, i) => (
              <div key={i} style={{ flex: 1, borderRadius: '4px 4px 0 0', transition: 'all 0.2s', height: `${(v / maxCheck) * 100}%`, background: v === maxCheck ? 'var(--accent)' : v === Math.min(...DAILY_CHECKS) ? 'rgba(216,140,154,0.4)' : 'rgba(252,174,145,0.3)', cursor: 'pointer' }}
