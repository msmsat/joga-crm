...
export default function DocumentsTab({ showToast }: { showToast: (msg: string, t?: ToastType) => void }) {
  ...
  const statusMeta: Record<string, { label: string; color: string; bg: string }> = {
    ...
    draft: { label: t('documents.status.draft'), color: 'var(--muted)', bg: 'rgba(var(--ink),0.06)' },
  };
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '14px' }}>
        <div style={{ fontSize: '14px', fontWeight: 800, color: 'var(--onyx)' }}>{t('tabs.documents')}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: 'var(--grid-gap)', marginBottom: 'var(--section-gap)' }}>
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('documents.signed')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>{signedCount}</div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('documents.pendingCount')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>
            <span style={{ color: pendingCount > 0 ? '#D88C9A' : 'var(--onyx)' }}>{pendingCount}</span>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('documents.draftCount')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>{draftCount}</div>
        </div>
      </div>

      {/* 2. Панель управления */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--grid-gap)', marginBottom: 'var(--section-gap)', background: 'var(--bg-card)', padding: '12px 16px', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 8px 32px -8px rgba(26,26,26,0.04)', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: '250px' }}>
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(var(--ink),0.06)', border: 'none', cursor: 'pointer', color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}><Ico.X /></button>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '4px', background: 'rgba(var(--ink),0.03)', padding: '4px', borderRadius: '12px', flexShrink: 0 }}>
          {(['all', 'signed', 'pending'] as const).map(f => {
            ...
            return (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: "'Manrope', sans-serif", background: isActive ? 'var(--bg-card)' : 'transparent', color: isActive ? 'var(--onyx)' : 'var(--muted)', boxShadow: isActive ? '0 2px 8px rgba(26,26,26,0.06)' : 'none', transition: 'all 0.2s' }}>
      ...
      {addOpen && (
        <div className={`${styles.morphContainer} card`} style={{ padding: '32px', marginBottom: '24px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 16px 40px -8px rgba(26,26,26,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-0.3px' }}>{t('documents.addTitle')}</div>
          ...
          <div className="split-tablet" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '32px' }}>
            ...
            >
              ...
              />
              <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: 'var(--bg-card)', boxShadow: '0 8px 24px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F9A08B', marginBottom: '16px' }}>
                <Ico.Download />
              </div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--onyx)', marginBottom: '4px' }}>{selectedFile ? selectedFile.name : t('documents.dropzoneTitle')}</div>
              <div style={{ fontSize: '12px', color: 'var(--text3)' }}>{selectedFile ? t('documents.dropzoneChange') : t('documents.dropzoneHint')}</div>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '8px' }}>{t('documents.titleLabel')}</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '8px' }}>{t('documents.partyLabel')}</label>
                <input type="text" placeholder={t('documents.partyPlaceholder')} value={newDoc.party} onChange={e => setNewDoc(p => ({ ...p, party: e.target.value }))} onFocus={() => setIsInputFocused({ ...isInputFocused, party: true })} onBlur={() => setIsInputFocused({ ...isInputFocused, party: false })} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg)', border: isInputFocused['party'] ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: 'var(--onyx)', outline: 'none', boxShadow: isInputFocused['party'] ? '0 0 0 3px rgba(249, 160, 139, 0.12)' : 'none', transition: 'all 0.2s', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'rgba(var(--ink),0.02)', borderRadius: '10px', border: '1px solid rgba(var(--ink),0.04)', marginTop: '8px' }}>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--onyx)' }}>{t('documents.needsSignature')}</div>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '2px' }}>{t('documents.needsSignatureHint')}</div>
                </div>
              ...
              <button onClick={handleCreate} disabled={!canSave} style={{ marginTop: 'auto', padding: '14px', background: canSave ? '#F9A08B' : 'rgba(var(--ink),0.04)', border: 'none', borderRadius: '10px', color: canSave ? '#FFFFFF' : 'var(--text3)', fontSize: '13px', fontWeight: 700, cursor: canSave ? 'pointer' : 'not-allowed', transition: 'all 0.2s', boxShadow: canSave ? '0 6px 20px rgba(249, 160, 139, 0.25)' : 'none' }}>
      ...
      {/* 4. Список документов */}
      <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', overflow: 'visible' }}>
        {filtered.length === 0 ? (
          <div style={{ padding: '64px 20px', textAlign: 'center', background: 'var(--bg2)' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(var(--ink),0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color: 'var(--text3)' }}><Ico.Doc /></div>
            <div style={{ fontSize: '16px', fontWeight: 800, color: 'var(--onyx)', marginBottom: '6px' }}>{t('documents.emptyTitle')}</div>
            <div style={{ fontSize: '13px', color: 'var(--muted)' }}>{t('documents.emptyBody')}</div>
          </div>
        ) : filtered.map((doc, i) => {
          const sm = statusMeta[doc.status];
          const extColor = extColors[doc.ext] || '#999';
          return (
            <div key={doc.id} className={`${styles.docRow} ${popoverDoc === doc.id ? styles.docRowActive : ''}`} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 24px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(var(--ink),0.12)' : 'none', background: 'transparent', transition: 'all 0.25s cubic-bezier(0.2, 0.8, 0.2, 1)', borderLeft: '3px solid transparent' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--onyx)', marginBottom: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{doc.title}</div>
                <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 500 }}>
                  <span style={{ color: 'var(--onyx)', fontWeight: 600 }}>{doc.party}</span> <span style={{ opacity: 0.5, margin: '0 4px' }}>•</span> {t('documents.uploaded', { date: doc.date })}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexShrink: 0, position: 'relative' }}>
                <button onClick={() => handleOpen(doc.id, doc.hasFile)} disabled={downloadingId === doc.id} style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'var(--bg)', border: '1px solid rgba(var(--ink),0.08)', cursor: downloadingId === doc.id ? 'default' : 'pointer', opacity: downloadingId === doc.id ? 0.6 : doc.hasFile ? 1 : 0.45, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', transition: 'all 0.2s', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--onyx)'; e.currentTarget.style.color = 'var(--onyx)'; e.currentTarget.style.transform = 'translateY(-1px)'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.transform = 'translateY(0)'; }} title={doc.hasFile ? t('documents.open') : t('documents.toasts.noFile')}><Ico.Open /></button>
                <button onClick={(e) => { e.stopPropagation(); const rect = e.currentTarget.getBoundingClientRect(); setPopoverUp(window.innerHeight - rect.bottom < 180); setPopoverDoc(popoverDoc === doc.id ? null : doc.id); }} style={{ width: '36px', height: '36px', borderRadius: '10px', background: popoverDoc === doc.id ? 'var(--onyx)' : 'var(--bg)', border: '1px solid rgba(var(--ink),0.08)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: popoverDoc === doc.id ? 'var(--bg)' : 'var(--muted)', transition: 'all 0.2s', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }} onMouseEnter={e => { if (popoverDoc !== doc.id) { e.currentTarget.style.borderColor = 'var(--onyx)'; e.currentTarget.style.color = 'var(--onyx)'; e.currentTarget.style.transform = 'translateY(-1px)'; } }} onMouseLeave={e => { if (popoverDoc !== doc.id) { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.transform = 'translateY(0)'; } }} title={t('documents.options')}><Ico.Dots /></button>
