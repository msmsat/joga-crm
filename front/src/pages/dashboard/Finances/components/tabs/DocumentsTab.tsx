          ...
          return (
            <div key={doc.id} className={`${styles.docRow} ${popoverDoc === doc.id ? styles.docRowActive : ''}`} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 24px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(var(--ink),0.12)' : 'none', background: 'transparent', transition: 'all 0.25s cubic-bezier(0.2, 0.8, 0.2, 1)', borderLeft: '3px solid transparent' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexShrink: 0, position: 'relative' }}>
                ...
                <button onClick={(e) => { e.stopPropagation(); const rect = e.currentTarget.getBoundingClientRect(); setPopoverUp(window.innerHeight - rect.bottom < 180); setPopoverDoc(popoverDoc === doc.id ? null : doc.id); }} style={{ width: '36px', height: '36px', borderRadius: '10px', background: popoverDoc === doc.id ? '#1A1A1A' : '#FDFCFB', border: '1px solid rgba(var(--ink),0.08)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: popoverDoc === doc.id ? '#FFFFFF' : 'var(--muted)', transition: 'all 0.2s', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }} onMouseEnter={e => { if (popoverDoc !== doc.id) { e.currentTarget.style.borderColor = 'var(--onyx)'; e.currentTarget.style.color = 'var(--onyx)'; e.currentTarget.style.transform = 'translateY(-1px)'; } }} onMouseLeave={e => { if (popoverDoc !== doc.id) { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.transform = 'translateY(0)'; } }} title={t('documents.options')}><Ico.Dots /></button>
