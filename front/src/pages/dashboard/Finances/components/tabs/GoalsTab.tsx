...
export default function GoalsTab({ showToast }: { showToast: (msg: string, t?: ToastType) => void }) {
  ...
  const gInp = (key: string): React.CSSProperties => ({
    ...
    border: editGFocused === key ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)',
  ...
  if (loading) {
    return <div style={{ padding: '64px 20px', textAlign: 'center', color: 'var(--text3)', fontSize: '14px', fontWeight: 600 }}>{t('goals.loading')}</div>;
  }

  return (
    <>
      {/* 1. Карточки сводки */}
      <div className={styles.goalSum} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: 'var(--grid-gap)', marginBottom: 'var(--section-gap)' }}>
        <div className={styles.goalSumCard} style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('goals.active')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>{activeGoals}</div>
        </div>
        ...
        <div className={styles.goalSumCard} style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('goals.done')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>{doneGoals}</div>
        </div>
        ...
        <div className={styles.goalSumCard} style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('goals.avgProgress')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>{avgProgress}%</div>
        </div>
      </div>

      {/* 2. Action island */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', background: 'var(--bg-card)', padding: '12px 16px', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 8px 32px -8px rgba(26,26,26,0.04)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', paddingLeft: '8px' }}>
          <div style={{ fontSize: '14px', fontWeight: 800, color: 'var(--onyx)' }}>{t('goals.trackingTitle')}</div>
      ...
      {/* 3. Форма создания */}
      {addOpen && (
        <div className={styles.morphContainer} style={{ padding: '32px', marginBottom: '24px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 16px 40px -8px rgba(26,26,26,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
            <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-0.3px' }}>{t('goals.newTitle')}</div>
          ...
          <div className="split-tablet" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '32px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <input type="text" placeholder={t('goals.titlePlaceholder')} value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} onFocus={() => setIsInputFocused({ ...isInputFocused, title: true })} onBlur={() => setIsInputFocused({ ...isInputFocused, title: false })} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg)', border: isInputFocused['title'] ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: 'var(--onyx)', outline: 'none', boxShadow: isInputFocused['title'] ? '0 0 0 3px rgba(249, 160, 139, 0.12)' : 'none', transition: 'all 0.2s', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: '16px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '8px' }}>{t('goals.targetLabel', { symbol: currency })}</label>
                  <input type="text" placeholder={t('goals.targetPlaceholder')} value={form.target} onChange={e => handleNumberInput(e.target.value, val => setForm(p => ({ ...p, target: val })))} onFocus={() => setIsInputFocused({ ...isInputFocused, target: true })} onBlur={() => setIsInputFocused({ ...isInputFocused, target: false })} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg)', border: isInputFocused['target'] ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: 'var(--onyx)', outline: 'none', boxShadow: isInputFocused['target'] ? '0 0 0 3px rgba(249, 160, 139, 0.12)' : 'none', transition: 'all 0.2s', boxSizing: 'border-box' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '8px' }}>{t('goals.deadlineLabel')}</label>
                  <input type="date" value={form.deadline} onChange={e => setForm(p => ({ ...p, deadline: e.target.value }))} onFocus={() => setIsInputFocused({ ...isInputFocused, deadline: true })} onBlur={() => setIsInputFocused({ ...isInputFocused, deadline: false })} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg)', border: isInputFocused['deadline'] ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: 'var(--onyx)', outline: 'none', boxShadow: isInputFocused['deadline'] ? '0 0 0 3px rgba(249, 160, 139, 0.12)' : 'none', transition: 'all 0.2s', boxSizing: 'border-box' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div onClick={() => setForm(p => ({ ...p, trackingMode: 'manual' }))} style={{ padding: '16px', borderRadius: '12px', border: form.trackingMode === 'manual' ? '2px solid #F9A08B' : '2px solid rgba(var(--ink),0.06)', background: form.trackingMode === 'manual' ? 'rgba(249,160,139,0.04)' : 'var(--bg)', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--muted)', lineHeight: 1.4 }}>{t('goals.trackingManualDesc')}</div>
                  </div>
                </div>
              </div>
              {form.trackingMode === 'auto' && (
                <div style={{ marginTop: '10px' }}>
                  ...
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '8px' }}>{t('goals.categoryLabel')}</label>
                  <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} onFocus={() => setIsInputFocused({ ...isInputFocused, category: true })} onBlur={() => setIsInputFocused({ ...isInputFocused, category: false })} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg)', border: isInputFocused['category'] ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: 'var(--onyx)', outline: 'none', boxShadow: isInputFocused['category'] ? '0 0 0 3px rgba(249, 160, 139, 0.12)' : 'none', transition: 'all 0.2s', boxSizing: 'border-box' }}>
                    <option value="">{t('goals.categoryPlaceholder')}</option>
                    {categoryOptions.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
              )}
              <button onClick={handleAdd} disabled={!form.title.trim() || !form.target} style={{ marginTop: 'auto', padding: '14px', background: form.title.trim() && form.target ? '#F9A08B' : 'rgba(var(--ink),0.04)', border: 'none', borderRadius: '10px', color: form.title.trim() && form.target ? '#FFFFFF' : 'var(--text3)', fontSize: '13px', fontWeight: 700, cursor: form.title.trim() && form.target ? 'pointer' : 'not-allowed', transition: 'all 0.2s', boxShadow: form.title.trim() && form.target ? '0 6px 20px rgba(249, 160, 139, 0.25)' : 'none', fontFamily: "'Manrope', sans-serif" }}>
                {t('goals.createGoal')}
              </button>
            </div>
          </div>
              ...
              {editingGoalId === g.id ? (
                <div key="edit" className={styles.morphContainer}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '14px' }}>
                    <div>
                      <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '5px' }}>{t('goals.nameLabel')}</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '5px' }}>{t('goals.amountLabel', { symbol: currency })}</div>
                      ...
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '5px' }}>{t('goals.deadlineLabel')}</div>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => setEditingGoalId(null)} style={{ padding: '8px 12px', background: 'transparent', border: '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--muted)', cursor: 'pointer', transition: 'all 0.2s', fontFamily: "'Manrope', sans-serif" }} onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(var(--ink),0.2)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'}>{t('common.cancel')}</button>
                  </div>
                </div>
              ) : (
                <div key="view">
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '16px', fontWeight: 800, color: 'var(--onyx)', marginBottom: '4px', lineHeight: 1.3 }}>{g.title}</div>
                      <div style={{ fontSize: '12px', color: 'var(--text3)', fontWeight: 500 }}>{t('goals.deadlinePrefix')} {g.deadline ?? t('goals.noDeadline')}</div>
                    </div>
                    <div style={{ display: 'flex', gap: '4px' }}>
                      <button onClick={() => openGoalEdit(g)} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid transparent', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(249,160,139,0.1)'; e.currentTarget.style.color = '#F9A08B'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text3)'; }} title={t('common.edit')}>
                        <Ico.Edit />
                      </button>
                      <button onClick={() => setConfirm({ open: true, id: g.id })} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid transparent', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--tint-rose)'; e.currentTarget.style.color = '#D88C9A'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text3)'; }} title={t('goals.deleteConfirmTitle')}>
                        <Ico.Trash />
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '8px' }}>
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--onyx)' }}>{fmt(g.current_amount)} <span style={{ color: 'var(--text3)', fontWeight: 500 }}>/ {fmt(g.target_amount)}</span></div>
                  </div>
                  <div style={{ height: '12px', background: 'rgba(var(--ink),0.04)', borderRadius: '12px', overflow: 'hidden', marginBottom: '20px', border: '1px solid rgba(var(--ink),0.02)' }}>
                  ...
                  {!done && (
                    <div style={{ borderTop: '1px dashed rgba(var(--ink),0.08)', paddingTop: '16px' }}>
                      {g.tracking_mode === 'manual' ? (
                        isFunding ? (
                          <div className={styles.morphContainer} style={{ display: 'flex', gap: '8px' }}>
                            <input
                              ...
                              style={{ flex: 1, padding: '10px 12px', background: 'var(--bg)', border: isFundFocused ? `1.5px solid ${color}` : '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: 'var(--onyx)', outline: 'none', transition: 'all 0.2s', boxSizing: 'border-box' }}
                            />
                            <button onClick={() => handleFund(g)} disabled={!fundAmount} style={{ padding: '0 16px', background: fundAmount ? color : 'rgba(var(--ink),0.04)', border: 'none', borderRadius: '8px', color: '#FFF', fontSize: '12px', fontWeight: 700, cursor: fundAmount ? 'pointer' : 'not-allowed', transition: 'all 0.2s', fontFamily: "'Manrope', sans-serif" }}>{t('goals.fund')}</button>
                            <button onClick={() => setFundGoalId(null)} style={{ width: '36px', background: 'transparent', border: '1px solid rgba(var(--ink),0.08)', borderRadius: '8px', color: 'var(--muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ico.X /></button>
                          </div>
                        ) : (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text3)', fontSize: '11px', fontWeight: 600 }}>
