...
export default function OperationsTab({ showToast, initialSearch }: {
  ...
  const inp = (key: string): React.CSSProperties => ({
    ...
    border: addFocused === key ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)',
    borderRadius: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--onyx)', outline: 'none',
  ...
  const einp = (key: string): React.CSSProperties => ({
    width: '100%', padding: '10px 12px', background: 'var(--bg)',
    border: eFocused === key ? '1.5px solid #F9A08B' : '1.5px solid rgba(var(--ink),0.08)',
    borderRadius: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--onyx)', outline: 'none',
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '28px' }}>
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.04)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden', transition: 'transform 0.2s', cursor: 'default' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'none'}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('operations.income')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.04)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden', transition: 'transform 0.2s', cursor: 'default' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'none'}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('operations.expense')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(var(--ink),0.04)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', position: 'relative', overflow: 'hidden', transition: 'transform 0.2s', cursor: 'default' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'none'}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{t('operations.balance')}</div>
          </div>
          <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--onyx)', letterSpacing: '-1px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px', background: 'var(--bg-card)', padding: '12px 16px', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.04)', boxShadow: '0 8px 32px -8px rgba(26,26,26,0.04)', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: '250px' }}>
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(var(--ink),0.06)', border: 'none', cursor: 'pointer', color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.background='rgba(var(--ink),0.1)'} onMouseLeave={e => e.currentTarget.style.background='rgba(var(--ink),0.06)'}>
              <Ico.X />
            </button>
          )}
        </div>

        <div style={{ display: 'flex', gap: '4px', background: 'rgba(var(--ink),0.03)', padding: '4px', borderRadius: '12px', flexShrink: 0 }}>
          {(['all', 'in', 'out'] as const).map(f => {
            const isActive = filter === f;
            return (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: "'Manrope', sans-serif", background: isActive ? 'var(--bg-card)' : 'transparent', color: isActive ? 'var(--onyx)' : 'var(--muted)', boxShadow: isActive ? '0 2px 8px rgba(26,26,26,0.06)' : 'none', transition: 'all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1)' }}>
        ...
        <button onClick={handleExport} disabled={exporting} style={{ display: 'flex', alignItems: 'center', gap: '8px', height: '40px', padding: '0 16px', background: 'transparent', border: '1.5px solid rgba(var(--ink),0.08)', borderRadius: '10px', color: 'var(--muted)', fontSize: '13px', fontWeight: 600, cursor: exporting ? 'default' : 'pointer', opacity: exporting ? 0.6 : 1, fontFamily: "'Manrope', sans-serif", transition: 'all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1)', flexShrink: 0 }} onMouseEnter={e => { if (exporting) return; e.currentTarget.style.background = 'rgba(var(--ink),0.03)'; e.currentTarget.style.color = 'var(--onyx)'; e.currentTarget.style.borderColor = 'rgba(var(--ink),0.15)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; }}>
        ...
        <div className={styles.morphContainer} style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1.5px solid #F9A08B', padding: '24px', marginBottom: '24px', boxShadow: '0 12px 28px rgba(249,160,139,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.titleLabel')}</div>
            ...
            {nType === 'in' && (
              <div style={{ gridColumn: '1 / -1', position: 'relative' }}>
                <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.clientLabel')}</div>
                {nClientId ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: 'rgba(249,160,139,0.06)', border: '1.5px solid #F9A08B', borderRadius: '8px' }}>
                    <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--onyx)' }}>{nClientQuery}</span>
                    <button type="button" onClick={() => { setNClientId(null); setNClientQuery(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', display: 'flex' }}>
                      <Ico.X />
                    </button>
                  </div>
                ) : (
                ...
                {!nClientId && clientResults && clientResults.items.length > 0 && (
                  <div style={{ position: 'absolute', zIndex: 5, top: '100%', left: 0, right: 0, marginTop: '4px', borderRadius: '8px', border: '1px solid rgba(var(--ink),0.08)', background: 'var(--bg-card)', boxShadow: '0 8px 24px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                    {clientResults.items.map(c => {
                      const name = c.last_name ? `${c.name} ${c.last_name}` : c.name;
                      return (
                        <button key={c.id} type="button" onClick={() => { setNClientId(c.id); setNClientQuery(name); }} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 12px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', color: 'var(--onyx)' }}>
                          {name}
                        </button>
                      );
                    })}
                ...
                <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '6px' }}>{t('operations.clientHint')}</div>
              </div>
            )}
            <div>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.amountLabel', { symbol: currency })}</div>
            ...
            <div>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.dateLabel')}</div>
            ...
            <div>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.methodLabel')}</div>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{t('operations.counterpartyLabel')}</div>
              <select
            ...
            {newCpOpen && (
              <div className={styles.morphContainer} style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', padding: '14px', background: 'rgba(249,160,139,0.04)', borderRadius: '10px', border: '1px dashed rgba(249,160,139,0.4)' }}>
                ...
                <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '8px' }}>
                  <button type="button" onClick={handleCreateCounterparty} disabled={!newCpName.trim() || savingCp} style={{ padding: '8px 14px', background: (newCpName.trim() && !savingCp) ? '#F9A08B' : 'rgba(var(--ink),0.06)', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: '#FFFFFF', cursor: (newCpName.trim() && !savingCp) ? 'pointer' : 'not-allowed' }}>{savingCp ? t('operations.saving') : t('common.create')}</button>
                  <button type="button" onClick={() => { setNewCpOpen(false); setNewCpName(''); }} style={{ padding: '8px 14px', background: 'transparent', border: '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--muted)', cursor: 'pointer' }}>{t('common.cancel')}</button>
                </div>
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: '10px', paddingTop: '14px', borderTop: '1px solid rgba(var(--ink),0.05)' }}>
            <button onClick={handleCreate} disabled={!nTitle.trim() || saving} style={{ padding: '10px 20px', background: (nTitle.trim() && !saving) ? '#F9A08B' : 'rgba(var(--ink),0.06)', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: '#FFFFFF', cursor: (nTitle.trim() && !saving) ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>{saving ? t('operations.saving') : t('common.create')}</button>
            <button onClick={() => { setAddOpen(false); resetForm(); }} style={{ padding: '10px 16px', background: 'transparent', border: '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: 'var(--muted)', cursor: 'pointer' }}>{t('common.cancel')}</button>
          </div>
        </div>
      )}
      ...
      {/* 3. Список операций */}
      {loading ? (
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '64px 20px', textAlign: 'center', color: 'var(--text3)', fontSize: '14px', fontWeight: 600, border: '1px solid rgba(var(--ink),0.06)' }}>{t('operations.loading')}</div>
      ) : operations.length === 0 ? (
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '64px 20px', textAlign: 'center', border: '1px dashed rgba(var(--ink),0.1)' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(var(--ink),0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color: 'var(--text3)' }}><Ico.Search /></div>
          <div style={{ fontSize: '16px', fontWeight: 800, color: 'var(--onyx)', marginBottom: '6px' }}>{t('operations.emptyTitle')}</div>
          <div style={{ fontSize: '13px', color: 'var(--muted)' }}>{t('operations.emptyBody')}</div>
        </div>
      ) : (
        <>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid rgba(var(--ink),0.12)', boxShadow: '0 12px 32px -4px rgba(26,26,26,0.02)', overflow: 'hidden' }}>
            {operations.map((op, i) => {
              ...
              return (
                <div key={op.id} style={{ borderBottom: i < operations.length - 1 ? '1px solid rgba(var(--ink),0.12)' : 'none' }}>
                  <div
                    ...
                    onMouseEnter={e => { if (!isOpen) e.currentTarget.style.background = 'rgba(var(--ink),0.01)'; }}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--onyx)', marginBottom: '3px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {op.title}
                        {op.status === 'pending' && (
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0, paddingRight: '12px' }}>
                      ...
                      <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '3px', fontWeight: 500 }}>{op.op_date}</div>
                    </div>
                    <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: isOpen ? 'rgba(var(--ink),0.06)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', transition: 'all 0.2s', transform: isOpen ? 'rotate(90deg)' : 'none', flexShrink: 0 }}>
                      <Ico.Chevron />
                    </div>
                  </div>
                    ...
                    <div style={{ borderTop: '1px solid rgba(252,174,145,0.1)' }}>
                      {editingOpId === op.id ? (
                        <div key="edit" className={styles.morphContainer} style={{ padding: '20px 24px', background: 'rgba(252,174,145,0.03)' }}>
                          ...
                          <div style={{ display: 'flex', gap: '10px', paddingTop: '14px', borderTop: '1px solid rgba(var(--ink),0.05)' }}>
                            <button onClick={saveOpEdit} disabled={!eTitle.trim() || savingEdit} style={{ padding: '10px 20px', background: (eTitle.trim() && !savingEdit) ? '#F9A08B' : 'rgba(var(--ink),0.06)', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: '#FFFFFF', cursor: (eTitle.trim() && !savingEdit) ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>{savingEdit ? t('operations.saving') : t('common.save')}</button>
                            <button onClick={() => setEditingOpId(null)} style={{ padding: '10px 16px', background: 'transparent', border: '1.5px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '13px', fontWeight: 700, color: 'var(--muted)', cursor: 'pointer' }}>{t('common.cancel')}</button>
                          </div>
                        </div>
                      ) : (
                        <div key="view" className={styles.morphContainer} style={{ padding: '20px 24px', background: 'rgba(252,174,145,0.03)', display: 'grid', gridTemplateColumns: cp ? '1fr 1fr 1fr 1fr' : '1fr 1fr 1fr', gap: '24px' }}>
                          ...
                          <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '10px', paddingTop: '16px', borderTop: '1px solid rgba(var(--ink),0.05)', marginTop: '-4px' }}>
                            <button onClick={() => showToast(t('operations.toasts.receiptDownloaded'), 'success')} style={{ padding: '8px 16px', background: 'var(--bg-card)', border: '1px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--onyx)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }} onMouseEnter={e => { e.currentTarget.style.borderColor = '#F9A08B'; e.currentTarget.style.color = '#F9A08B'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; e.currentTarget.style.color = 'var(--onyx)'; }}><Ico.Doc /> {t('operations.receipt')}</button>
                            <button onClick={() => openOpEdit(op)} style={{ padding: '8px 16px', background: 'var(--bg-card)', border: '1px solid rgba(var(--ink),0.08)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: 'var(--onyx)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s', boxShadow: '0 2px 6px rgba(0,0,0,0.02)' }} onMouseEnter={e => { e.currentTarget.style.borderColor = '#F9A08B'; e.currentTarget.style.color = '#F9A08B'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.08)'; e.currentTarget.style.color = 'var(--onyx)'; }}><Ico.Edit /> {t('common.edit')}</button>
                            <button onClick={() => handleDelete(op.id)} style={{ padding: '8px 16px', background: 'transparent', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 700, color: '#D88C9A', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s', marginLeft: 'auto' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--tint-rose)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}><Ico.Trash /> {t('common.delete')}</button>
                          </div>
                        </div>
                      )}
                    </div>
          ...
          {operations.length < total && (
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
              <button onClick={loadMore} disabled={loadingMore} style={{ padding: '12px 28px', background: 'var(--bg-card)', border: '1.5px solid rgba(var(--ink),0.1)', borderRadius: '12px', fontSize: '13px', fontWeight: 700, color: 'var(--onyx)', cursor: loadingMore ? 'default' : 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => { if (!loadingMore) e.currentTarget.style.borderColor = '#F9A08B'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(var(--ink),0.1)'; }}>
