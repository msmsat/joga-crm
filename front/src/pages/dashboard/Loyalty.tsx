...
const LineChart = ({ data, color, valueKey }: { data: typeof chartData; color: string; valueKey: 'revenue' | 'clients' }) => {
  ...
  const range = max - min || 1;
  const W = 300; const H = 55; const pad = 10;
  const LABEL_H = 18;
  ...
  const areapts = `${getX(0)},${H} ` + pts + ` ${getX(values.length - 1)},${H}`;
      ...
      {values.map((v, i) => {
        ...
        return (
          <g key={i}>
            ...
            {isHov && (
              <line x1={cx} y1={0} x2={cx} y2={H} stroke={color} strokeWidth="1" strokeDasharray="3 3" opacity="0.4" />
            )}
            {/* Точка */}
            <circle cx={cx} cy={cy} r={isHov ? 5 : 3} fill={color} opacity={isHov ? 1 : 0.6}
              style={{ transition: 'r 0.1s' }} />
            {/* Тултип над точкой */}
            {isHov && (
              <g>
                <rect
                  x={cx - 28} y={cy - 26} width={56} height={20}
                  rx="5" fill={color} opacity="0.95"
                />
                <text x={cx} y={cy - 11} textAnchor="middle" fontSize="10" fontWeight="700" fill="white">
                  {formatValue(v)}
                </text>
              </g>
            )}
