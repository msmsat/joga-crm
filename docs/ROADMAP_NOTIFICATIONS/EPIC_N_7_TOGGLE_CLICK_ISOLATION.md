...
**Статус: ✅ реализовано.** Все 4 задачи найдены уже в рабочем дереве (без
коммита) при аудите перед стартом N-8. `npm run build`/`lint` по затронутым
файлам — чисто. Живые сценарии (клик мышью, Tab/Enter/Space, скринридер) не
прокликивал в браузере — см. пояснение по каждой задаче ниже.
...
✅ Подтверждено в [`ToggleSwitch.tsx`](../../front/src/pages/dashboard/Notifications/components/ui/ToggleSwitch.tsx) — совпадает с черновиком (`type="button"`, `onClick` с `stopPropagation`, `disabled`, `aria-label`, `role="switch"`).
...
✅ Подтверждено в [`ChannelsSidebar.tsx`](../../front/src/pages/dashboard/Notifications/components/sections/ChannelsSidebar.tsx) — `role`/`tabIndex`/`aria-label`/`onKeyDown` на строке, ключ `channels.openSettings` в обеих локалях. Реализация даже строже черновика: добавлена проверка `e.target !== e.currentTarget` в `onKeyDown` — без неё Enter/Space на вложенном тумблере/кнопке «Подключить» всплывал бы как keydown на строку и тоже открывал модалку (клик это перекрывает через `stopPropagation` в кнопках, а keydown — нет, отдельного `stopPropagation` там нет).
...
✅ Подтверждено — `channelSaving` пробрасывается `useNotifications` →
`Notifications.tsx` → `ChannelsSidebar` → `ToggleSwitch.disabled`.

### Задача 4 · Регресс-тест на изоляцию клика · 🟢 · 0:30 · ✅ (осознанно без теста — см. ниже)
...
✅ Проверено: `grep` по `package.json` — ни `vitest`, ни `@testing-library`
не установлены, файла-теста нет. Ровно случай, для которого эпик сам
предписал не заводить тест — пункты 1-4 Acceptance Criteria остаются на
ручную проверку (см. таблицу приёмки ниже).
...
|---|---|---|---|
| 1 | Telegram подключён. Клик **по тумблеру** Telegram | Тумблер переключился, **модалка НЕ открылась** ← главный критерий | 🔍 `stopPropagation` в `ToggleSwitch.onClick` — код гарантирует, не кликано вживую |
| 2 | Клик по иконке / названию / подписи строки Telegram | Модалка настройки открылась, **тумблер не переключился** | 🔍 клик на строке не проходит через кнопку тумблера — код верен, не кликано вживую |
| 3 | То же для Email и WhatsApp | Поведение идентично | 🔍 логика не зависит от конкретного канала |
| 4 | Быстрые 5 кликов по тумблеру | Ни одной модалки; в Network — запросы не наслаиваются (тумблер `disabled` во время PATCH) | 🔍 `disabled={channelSaving}` блокирует повторный клик, не смотрел DevTools |
| 5 | Канал НЕ подключён (видна кнопка «Подключить») | Клик по кнопке открывает модалку; клик по строке — тоже | 🔍 кнопка «Подключить» и сама строка оба зовут `onOpenModal`, код совпадает |
| 6 | Tab до строки канала, Enter | Открылась модалка | 🔍 `onKeyDown` на строке — не проверено клавиатурой вживую |
| 7 | Tab до тумблера, Space | Тумблер переключился, **модалка не открылась**, страница не проскроллилась | 🔍 `<button>` тумблера сам обрабатывает Space, `e.target !== e.currentTarget` в строке гасит всплытие — не проверено вживую |
| 8 | Скринридер / DevTools Accessibility на тумблере | `role="switch"`, `aria-checked` соответствует состоянию, есть `aria-label` | ✅ статически в коде — все три атрибута на месте |
| 9 | Instagram / SMS / Push (без `MODAL_KEY`) | Клик по строке ничего не открывает, тумблер работает — регресса нет | ✅ `requiresIntegration = MODAL_KEY[ch.key] !== undefined` — для этих ключей `onClick`/`role`/`tabIndex` на строке не навешиваются вовсе |
| 10 | После переключения тумблера F5 | Состояние сохранилось (зависимость от эпика N-6) | ✅ N-6 подтверждён отдельно (см. `EPIC_N_6_PROVIDER_STATE_PERSISTENCE.md`) |
| 11 | `cd front && npm run build && npm run lint` | Без ошибок в затронутых файлах | ✅ прогнано |
...
| 4 | Регресс-тест на всплытие | Фронт | 🟢 | 0:30 | ✅ нет Vitest — по правилу эпика тест не заводим |
