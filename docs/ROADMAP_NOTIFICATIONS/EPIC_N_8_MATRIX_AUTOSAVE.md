...
✅ Добавлено рядом с `updateEventToggle` без изменений.
...
✅ **Реализовано** в [`useNotifications.ts`](../../front/src/pages/dashboard/Notifications/hooks/useNotifications.ts)
ровно по черновику выше — `saveTogglesMut` теперь дёргает
`notificationsApi.bulkUpdateEventToggles` (был `Promise.all(map(updateEventToggle))`),
рядом добавлен `useEffect` с `DEBOUNCE_MS=600`. Все 4 тонкости учтены как
описано. **Намеренно не тронуто** (это Задача 6): кнопки «Сохранить»/«Отменить»
и `saveChanges`/`cancelChanges` в хуке пока остаются — на переходный период
между Задачей 4 и Задачей 6 автосохранение и ручная кнопка используют одну и
ту же `saveTogglesMut`, так что ручной клик во время дебаунса — это просто
дублирующий (идемпотентный, `ON CONFLICT`) bulk-запрос, а не гонка данных;
кнопки уйдут в Задаче 6. Build (`tsc -b && vite build`) — зелёный; lint по
изменённым файлам (`useNotifications.ts`, `notifications.api.ts`) — чисто
(в проекте отдельно есть ~90 preexisting lint-ошибок в Staff/Settings/auth.ts,
не связанных с этим эпиком — не трогал).
...
✅ **Реализовано иначе, чем оба варианта выше.** Проверил
[`client.ts`](../../front/src/api/client.ts) — авторизация действительно
заголовком (`Authorization: Bearer <token>` из `localStorage`, без cookie),
`sendBeacon` отпадает. Но вместо даунгрейда до confirm-промпта нашёлся третий
вариант, который в черновике не рассматривался: **`fetch(..., { keepalive:
true })`** — в отличие от `sendBeacon` поддерживает свои заголовки (включая
`Authorization`), в отличие от обычного `fetch` переживает выгрузку документа
(лимит ~64 КБ на все keepalive-запросы разом — с головой хватает для диффа
тумблеров). Добавлено:
- [`client.ts`](../../front/src/api/client.ts): `patchKeepalive(path, body)` —
  ручной токен + keepalive-fetch, рядом с уже существующими `downloadFile`/
  `openFile` (та же идиома проекта: свой fetch в обход `request()`, когда
  нужна возможность браузера, которой нет у обычного запроса).
- [`notifications.api.ts`](../../front/src/api/notifications/notifications.api.ts):
  `flushEventTogglesOnUnload`.
- [`useNotifications.ts`](../../front/src/pages/dashboard/Notifications/hooks/useNotifications.ts):
  `pendingRef` синхронизируется в отдельном `useEffect` (не в теле рендера —
  прямая запись в ref во время рендера запрещена линтером проекта, см. ту же
  оговорку у `syncedTogglesData`), `beforeunload`-хендлер и cleanup на unmount
  зовут один и тот же флаш.

Раз keepalive-запрос надёжно доезжает вместе с заголовком, нативный
`beforeunload`-prompt («точно хотите уйти?») не добавлял — с автосохранением
он был бы враньём (данные и так улетят), а UX «страшного» диалога никто не
просил.
...
✅ **Реализовано.** Три состояния в `NotificationMatrix.tsx`: `syncing =
saving || isDirty` → спиннер (тот же паттерн, что `Button.tsx` — border-круг +
`@keyframes ... spin`) + «Сохранение…»; иначе `saveFailed` → `Icon.AlertTriangle`
+ «Не сохранено» (`#D88C9A`); иначе `Icon.Check` + «Все изменения сохранены»
(`#A3C9A8`). Отклонение от таблицы в задаче: `syncing` включает и `isDirty`
(не только `saving`) — иначе в окне дебаунса (клик → ещё 600 мс до факти­ческого
запроса) статус ошибочно показал бы «Все изменения сохранены», хотя изменение
ещё не ушло на сервер. `saveFailed` — новое поле состояния в хуке
(`useState`), взводится в `onError` дебаунс-мутации и снимается в момент
следующей попытки сохранения (не при каждом клике — иначе индикатор ошибки
пропадал бы раньше, чем реально стартует новый запрос). Кнопки «Сохранить»/
«Отменить» и `saveChanges`/`cancelChanges` удалены из хука целиком (не просто
убраны из возврата — грепом проверил, что больше нигде не используются).
Неиспользуемые i18n-ключи `matrix.save`/`matrix.cancel` тоже убраны (ru/en).
Build + lint по изменённым файлам — чисто.

### Задача 7 · Сохранение при переключении роли · 🟢 · 0:15 · ✅

**Файл:** [`useNotifications.ts:166-167`](../../front/src/pages/dashboard/Notifications/hooks/useNotifications.ts#L166-L167)
...
✅ **Подтверждено чтением кода** — `switchRole` (строки 166-168) вызывает
только `setActiveRole`, не трогает `toggles`/`savedToggles`; debounce-эффект
для автосохранения ключуется по `[toggles, savedToggles]`, не по
`activeRole` — переключение роли никак не прерывает и не сбрасывает уже
тикающий таймер автосохранения. Правок не потребовалось, как и предполагал
эпик.
...
|---|---|---|---|
| 1 | Роль «Клиент», клик по одной галочке, подождать 1 с | В Network **один** `PATCH /settings/notifications/events/bulk`, тело `{"toggles":[{…}]}`, ответ 200 | 🔍 логика кода (debounce на `[toggles, savedToggles]`) — не кликал в живом браузере |
| 2 | F5 | Галочка в том же состоянии ← главный критерий эпика | 🔍 бэк персистит (тест), фронт гидрирует из `togglesQ.data` — механизм не менялся с N-6 |
| 3 | Повторить для ролей «Тренер», «Администратор», «Владелец» | Каждая сохраняется и переживает F5 | 🔍 один и тот же путь для всех ролей, ролеспецифичной логики нет |
| 4 | 10 быстрых кликов по разным галочкам за < 600 мс | **Один** bulk-запрос, в теле все 10 изменений | 🔍 каждый клик перезапускает `setTimeout`, cleanup гарантирует один финальный |
| 5 | «Активировать все» при 3 активных каналах, роль «Клиент» | **Один** запрос с ~30 тумблерами (не 30 запросов) | 🔍 один синхронный `setToggles`, один re-render, один таймер |
| 6 | Кликнуть галочку и сразу переключить роль | Изменение всё равно ушло; вернулся на роль — галочка на месте | ✅ прочитан код `switchRole` — не трогает `toggles`/`savedToggles` (Задача 7) |
| 7 | Кликнуть галочку и сразу уйти на другую страницу | Флаш на unmount отработал; вернулся — галочка сохранена | 🔍 `pendingRef` + unmount-cleanup зовут keepalive-флаш — логика проверена чтением, не сквозным прогоном |
| 8 | DevTools → Network offline → клик по галочке | Тост об ошибке, галочка **вернулась** в предыдущее состояние (откат) | 🔍 `onError`: `setToggles(savedToggles)` + `setSaveFailed(true)` + toast — не воспроизводил живым отключением сети |
| 9 | `SELECT * FROM notification_event_toggles WHERE studio_id=<id>` после серии кликов | Строки по всем 4 ролям, **без дублей** по `(role, event_id, channel_key)` | ✅ `test_bulk_upsert_inserts_then_updates` — прямой `SELECT` в реальной БД после двух вызовов |
| 10 | Дважды прогнать один и тот же bulk-payload | Количество строк не выросло (`ON CONFLICT` отработал) | ✅ тот же тест: 2 вызова с одним ключом → 2 строки, не 3 |
| 11 | bulk с `"role": "hacker"` | `422`, ничего не записано | ✅ `test_bulk_rejects_unknown_role` (`ValidationError` на уровне Pydantic = 422 у FastAPI) |
| 12 | bulk с `studio_id` чужой студии в теле | Поле игнорируется — `studio_id` берётся из `ctx`; записи ушли в свою студию | ✅ в `EventToggle`/`EventToggleBulkUpdate` нет поля `studio_id` вовсе; роут берёт его только из `ctx.studio_id` |
| 13 | bulk под ролью `admin` | `403` | ✅ `test_bulk_requires_owner` |
| 14 | Матрица в UI | Кнопок «Сохранить»/«Отменить» нет; есть «Сохранение…» → «Все изменения сохранены» | 🔍 код: кнопки удалены, статус на месте — не открывал страницу в браузере |
| 15 | `cd back && python -m tests.test_notification_event_toggles` | Проходит (в проекте нет pytest — см. Задача 2) | ✅ `ALL PASS`, прогнан |
| 16 | `cd front && npm run build && npm run lint` | Без ошибок в затронутых файлах | ✅ build чист; lint по изменённым файлам чист (в репо отдельно ~90 доepic-овых ошибок в Staff/Settings/auth.ts) |

**Легенда:** ✅ — прогнано автоматически (тест/build/чтение кода, где логика
безальтернативна). 🔍 — проверено чтением кода и рассуждением о потоке
данных, но не кликано в живом браузере (по правилу проекта дев-сервер/браузер
не поднимается без явного запроса на визуальную проверку). Пункты 1, 4-5, 7-8,
14 стоит один раз прогнать глазами в браузере перед тем, как считать эпик
полностью закрытым — там, где заявлена жёсткая проверка через DevTools
Network/offline, самостоятельно её не выполнял.
