...
# Mini-app «Записатись»: мастера без обязательной услуги (14.09.2026)

Исполнитель: Claude (Opus 5). Задача владельца: услуга — фильтр мастеров, а не
первый шаг; постоянного календаря над мастерами нет; день и время — в листе
после выбора мастера.

## Что было

`BookingFlow` требовал сначала услугу и брал мастеров из `/staff-day` на день из
верхней `WeekRail`: мастер без смены в этот день пропадал из списка. Лист времени
снова рисовал `WeekRail` и один день слотов, ошибки уходили в `alert`.

## Backend

- **`GET /global/resource-staff?branch_id=&service_id=`** (read-only, гость
  допускается, 60/мин). Мастера филиала и ВСЕ их услуги, на которые можно
  записаться. `service_id` сужает мастеров, а не их `service_ids`. Один SQL на
  всех мастеров (`resource_availability.resource_staff`).
  `/staff-day` не расширялся: у него день и смены в контракте.
- **Одно правило «кто принимает запись»** — `_eligible_staff` (активный
  `trainer` + назначение на филиал). Им пользуются и `load` (availability, quote,
  confirm), и новый список: две копии разошлись бы.
- **Найден и исправлен 500 каталога:** `ServiceInfo.service_type: str`, а колонка
  и `ServiceCreate` допускают NULL — `GET /global/studio` падал целиком для
  студии с такой услугой (в dev-БД таких 31 из 55, все у тестовых студий).
  Поле стало `Optional`.
- `/staff-day` в мини-приложении больше не используется; ручка и её тесты
  оставлены как стабильный публичный контракт.

## Mini-app

- `pages/shedule.tsx` — развилка по `booking_mode`: `resource` → `BookingPage`,
  `event` → `pages/schedule/EventSchedule.tsx` (прежний код без изменений
  поведения), `hybrid` → переключатель `ModeSwitch` над двумя разделами.
- `pages/booking/BookingPage.tsx`: филиал → чипы услуг («Усі послуги» + услуги
  филиала) → «Будь-який майстер» → карточки мастеров (фото/инициалы, должность,
  до трёх услуг + «ще N», подсказка ближайшего времени при выбранной услуге —
  один `availability` без `teacher_id` на всех).
- Состояние — чистый автомат `lib/bookingPage.ts` (reducer + `reconcile`), даты —
  `lib/slots.ts` (день студии через `Intl` в её поясе, время срезом `local_start`).
- `BookingSheet` — ОДИН лист на шаги «услуга мастера → день/время → условия →
  итог»; назад к услугам — кнопкой в шапке листа (`Sheet.onBack`).
- `useResourceBooking` грузит время страницами по 14 дней в пределах
  `booking_window_days`: лента дней с отметкой «есть время», ближайший свободный
  день выбирается сам. Коды отказов (`SLOT_UNAVAILABLE`, `QUOTE_EXPIRED`, …) — в
  листе текстом по коду, не `alert`; `TERMS_CHANGED`/`QUOTE_EXPIRED` при confirm
  пересчитывают quote того же слота и требуют нового подтверждения (MA-06).
- Удалены мёртвые `BookingFlow.tsx`, `useStaffDay.ts`, `utils.hhmm`, ключи
  локалей старого экрана; новые строки — во всех 5 языках.

## Проверки

| Проверка | Результат |
|---|---|
| `pytest tests/test_resource_staff.py` (новый, 5 тестов, HTTP через ASGI) | 5 passed |
| Затронутые рефакторингом `test_staff_day`, `test_resource_availability`, `test_resource_booking`, `test_hybrid_crm_api` | 20 passed |
| Полный backend, первый прогон (параллельно с браузером и uvicorn, 63 мин) | 1848 passed, 3 failed (`test_identity`, `test_inbound_events`, `test_subscription_pending`); все три отдельно — 13 passed |
| Полный backend, чистый прогон (16 мин) | **1850 passed, 1 failed** — `test_ai_action.py::test_ai_action_confirmation` (групповая запись через ассистента, мимо изменённого кода); отдельно — 1 passed |
| Mini-app `npm run build` / `npm run lint` | ✅ / 0 ошибок, 1 предсуществующее предупреждение (`App.tsx`) |
| Mini-app `bookingPage.check.ts` (новый, 19 проверок) и все прежние `*.check.ts` | все ✅ |

Браузер (Playwright + Chrome, 375×667 и 1280×720, локальный uvicorn + dev-БД,
студия FIGARO): гость видит мастеров без услуги; фильтр; путь «услуга → мастер»
и «мастер → его услуга» до реальной брони (сверено с `reservations`/`lessons`:
`teacher_id`, `service_id`, `branch_id`, `start_time` = показанный `local_start`),
«Мои занятия», «любой мастер» без `teacher_id`, 402 без абонемента → лист
абонемента, ошибки/пустые состояния/скелет через подмену ответов, смена филиала,
hybrid, регрессия event на AURA. Две тестовые брони отменены, абонемент вернулся.

Второй круг (после чистого прогона): гость на выборе времени → вход; главная →
филиал → лист «любой мастер»; confirm с подменённым `TERMS_CHANGED` → новый quote
с пояснением → настоящая бронь; подменённый `SLOT_UNAVAILABLE` → возврат ко времени
с `role="alert"` и перечитанным временем; перенос из «Моих записей»
(`reschedule-quotes` 201 → `reschedule` 200, та же бронь, версия занятия 2 — сверено
с БД); uk/en/cs/de без сырых ключей; групповая запись на AURA открывает прежний
лист мест. Тестовая бронь отменена, абонемент вернулся.

Наблюдение, не изменённое намеренно: перенос из «Моих записей» открывается с
«любым мастером» (как и до этой задачи), и назначенный мастер может смениться —
он показывается в условиях до подтверждения.

**Не проверено:** Safari на iPhone и вебвью Instagram на живом устройстве.
