...
- [x] В `Overview/hooks/` нет ни одного `useEffect` с фетчем; всё через `useQuery`.
- [x] Свернуть вкладку → вернуться: summary и лента обновляются молча, без мигания и без F5 —
      подтверждено чтением кода (`refetchOnWindowFocus: true` в `queryClient.ts`, per-виджетные
      скелетоны вместо общего лоадера); **живая проверка в браузере не проводилась**.
- [x] Скрытая вкладка не шлёт запросов (проверка в Network) — встроенное поведение
      React Query (`refetchInterval` не тикает при `document.hidden`); Network не открывался.
- [x] Смена метрики и периода не гасит график (`keepPreviousData` работает).
- [x] Переход «Обзор → Клиенты → Обзор» в пределах 30 с не вызывает повторных запросов —
      `summary`/`trainers`/`services` держат `staleTime: 5 мин`, `activity` — дефолтные 30 с
      из `queryClient.ts`.
- [x] Комментарий-реестр в `queryKeys.ts` обновлён (Обзор больше не в очереди миграции).
- [x] `cd front && npm run build && npm run lint` — чисто (0 ошибок/предупреждений в файлах
      Обзора; существующие ошибки лежат в `Staff/*` и `utils/auth.ts`, не относятся к эпику).

**Дополнение к Задаче 3 (первый заход не закрывал контракт ошибок).** Хук уже был
переписан на `useQuery`, но состояние ошибки нигде не всплывало: экран просто
рисовал нули/пустые списки при сбое сети. Добавлено по образцу `useSchedule.ts`/
`Journal.tsx`:
- `useOverviewData` отдаёт `loadError`, `isFirstLoadError`, `refetchAll` (403 туда
  не попадает — это `forbidden`, отдельная ветка).
- `Overview.tsx`: `isFirstLoadError` → `EmptyState` с кнопкой «Повторить» вместо
  графика/метрик/ленты/сводок; фоновая ошибка (кэш уже есть) → тихий `toast.error`
  через `useToast`, без потери контента с экрана.
- Это добавило в `Overview.tsx` один `useEffect` (тост при смене `loadError`) —
  чек-лист Задачи 4 («`grep useEffect` → пусто») в буквальном виде больше не
  выполняется, но нарушения его духа нет: эффект ничего не фетчит и не дублирует
  `useQuery`, а синхронизирует тост с уже загруженными данными — тот же паттерн,
  что в живом `Journal.tsx:139-144`.
