...
- [x] Заголовок — «Задачи» / «Tasks»; строки «на сегодня» нет ни в коде, ни в словарях.
- [x] `owner` видит 3 опции первого дропдауна, `admin` — 2, `trainer` — дропдаунов нет.
- [x] Выбор «Тренеров» открывает второй дропдаун с именами; выбор имени фильтрует список.
- [x] Задача, созданная в режиме «Тренеров → Игорь», уходит с `assignee_id` Игоря — подтверждено чтением кода (`AddTaskForm.tsx` собирает `assignee_id` из выбранного `assigneeId`, бэк `POST /analytics/tasks` его принимает); **живая проверка в Network/БД не проводилась** — браузер/бэкенд не поднимались в этой сессии.
- [x] В чужом срезе без выбранного сотрудника кнопка «Добавить» заблокирована.
- [x] Отметка выполнения мгновенная, при ошибке сети откатывается и показывает тост.
- [x] Переключение скоупа/сотрудника и все мутации — без единой перезагрузки страницы.
- [x] Админ и тренер открывают Обзор и видят свой виджет задач вместо заглушки на весь экран.
- [x] Ни одного самописного дропдауна: только `Select` из `components/ui/index`.
- [x] `TasksWidget.tsx` и каждый файл в `widgets/tasks/` — меньше 200 строк.
- [x] `cd front && npm run build && npm run lint` — чисто.

**Отступление от текста эпика (согласовано с пользователем в сессии реализации):**
подзаголовок шапки показывает срез («Мои» / имя сотрудника), а не
плюрализованный счётчик `tasks.remaining` — сам эпик в двух соседних абзацах
даёт противоречащие инструкции («подзаголовок = remaining count» и тут же
«не дублировать число, бейдж и так его показывает»); выбрана вторая, более
обоснованная версия.
