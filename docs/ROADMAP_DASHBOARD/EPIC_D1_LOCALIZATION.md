...
- [x] `grep -rn "₽" front/src/pages/dashboard/Overview` → пусто (единственное вхождение — в JSDoc-комментарии `MetricCard.tsx`, не в коде/UI).
- [x] `grep -rn "'—'\|\"—\"" front/src/pages/dashboard/Overview` → пусто.
- [x] `grep -rn "ru-RU" front/src/pages/dashboard/Overview` → пусто.
- [x] Переключение языка RU↔EN перерисовывает **все** подписи Обзора без перезагрузки —
      подтверждено чтением кода: все строки резолвятся через `t()` внутри рендера
      (`useTranslation('dashboard')` подписывает компонент на `languageChanged`);
      модульные константы `PERIOD_LABEL`/`PERIOD_SUB` (AnalyticsChart) и заголовки
      `METRIC_PRESENTERS.title` перенесены внутрь компонента/хука, как требовала Задача 4;
      **живая проверка в браузере не проводилась** — браузер не поднимался в этой сессии.
- [x] Студия с валютой `USD` → `$` во всех денежных полях (метрика, тултип бара, сводка) —
      `getCurrencySymbol(code)` (`components/UI.tsx:482`) резолвит символ по коду валюты
      универсально для любой студии; все денежные значения идут через `fmtMoneyCompact(v, currencySymbol)`.
- [x] Студия без данных за месяц: метрики показывают `0 ₽ / 0 / 0 / 0 %`, чипы трендов скрыты, списки — «Нет данных» —
      `buildMetrics` берёт `s?.field ?? 0`, `MetricCard` рендерит чип тренда только при
      `changePct !== null`, `SummaryWidgets` — `t('state.noData')` при пустых списках услуг/тренеров.
- [x] Отрицательный тренд красится как падение (`stat-change down`), а не как рост —
      `MetricCard.tsx`: `` `stat-change ${metric.changePct >= 0 ? 'up' : 'down'}` ``.
- [x] `cd front && npm run build && npm run lint` — чисто в файлах Обзора (0 ошибок/предупреждений,
      проверено `eslint` точечно по `Overview/**`); существующие ошибки в `Staff/*`, `Settings/*`,
      `Loyalty/*`, `utils/auth.ts` — не относятся к эпику (те же файлы, что отмечены в D5).

**Уточнение по ходу Задачи 3.** Ключ `state.empty` уже был занят под конкретный смысл
(«в студии нет таких сотрудников» — фильтр исполнителей в `TasksHeader`, заведён в D4).
Для общего «Нет данных» (списки услуг/тренеров в `SummaryWidgets`) заведён отдельный
ключ `state.noData` — по аналогии с уже существующим `chart.noData`, вместо переопределения
`state.empty` и поломки его текущего использования.

**Отступление от текста эпика.** Строка `activeConfig.title.replace(' за месяц', '').replace(' сегодня', '')`
в `AnalyticsChart.tsx` не тронута: это не самостоятельный русский литерал, отображаемый
пользователю, а фрагмент логики построения короткого заголовка графика (не входит в
таблицу нарушений эпика для `AnalyticsChart.tsx` и не ловится ни одним grep-пунктом DoD).
Для английской локали замены не сработают (нет ` за месяц`/` сегодня` в EN-строках),
результат — не поломка, а безобидная избыточность («Revenue this month by week»).
Отдельные ключи под короткие заголовки не заводились, чтобы не плодить словарь под
косметику вне явного скоупа задачи — кандидат в `docs/BACKLOG`, если понадобится.
