...
- [x] 5 эндпоинтов `/ai/sessions*` на реальной БД, скоуп `studio_id` +
      `user_id` на каждом (чужая/несуществующая сессия — 404, не 403);
      `message_count` одним запросом (outerjoin+group_by), не N+1.
- [x] `services/assistant.py`: без `LLM_BASE_URL` — честная локализованная
      заглушка (ru/en); с `LLM_BASE_URL` — реальный OpenAI-совместимый payload,
      таймаут 60с, `503 assistant_unavailable` на сбое; код роутера при
      подключении модели не меняется.
      Отличие от буквы задачи 3: `generate_reply` получил параметр
      `studio_language` — без него нельзя было выполнить требование
      «`auto` → язык студии, fallback `ru`» (контракт в задаче не передавал
      `Studio`, а мутировать `settings.language` для резолва было нельзя —
      ORM-объект, попал бы в commit).
- [x] Одна транзакция на пару сообщений (`flush()`, не `commit()`, до генерации
      ответа) — при падении LLM вопрос без ответа не остаётся в истории.
- [x] `python -m tests.test_ai_chat` зелёный (pytest не установлен в venv —
      запуск через конвенцию репо, как у соседних тестов): полный цикл
      создание → 2 сообщения → GET messages/sessions → DELETE → 404 →
      изоляция по пользователю; отдельно проверен каскад удаления сообщений.
- [x] `useAssistant` — единый движок чата на TanStack Query, `sessions`/
      `messages` только из кэша (`queryKeys.aiSessions`/`aiMessages(id)`),
      оптимистичная отправка с откатом на ошибке, глобальный тост.
- [x] `activeSessionId` поднят в `AIDrawerContext` — общий для страницы AI,
      дровера и AI-строки шапки; вопрос из любой поверхности без F5 виден на
      двух других из одного кэша.
- [x] Мок-данные выпилены: `grep MOCK_` в `pages/dashboard/AI/` и
      `components/AIDrawer/` — пусто.
- [x] `cd front && npm run build && npm run lint` — build зелёный; lint правок
      эпика чист (94 → 91 проблем в репо — снижение за счёт эпика, остаток вне
      его файлов, включая 1 pre-existing `react-refresh` в
      `AIDrawerContext.tsx`, не тронутую этим изменением).
