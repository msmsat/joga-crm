...
| Локализация + символ валюты из глобального стейта | ✅ Done. `formatMoney`/`localeForCurrency` (`front/src/lib/money.ts`) + namespace `billing` (`front/src/locales/{ru,en}/billing.json`); хардкод `₽`/`ru-RU` убран из `PlansTab`/`InvoicesTab`/`PaymentMethodTab`/`SavingsIllustration`/`UpgradeModal`/`BillingHeader` | Epic B0 |
