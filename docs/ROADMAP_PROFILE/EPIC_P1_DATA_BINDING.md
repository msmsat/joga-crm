...
## Задача 1. Удалить локальные копии кита (~0:30)
...
## Задача 2. Личные данные → TanStack Query + фамилия + честный email (~1:00)
...
## Задача 3. «Связанные аккаунты» → реальные студии (~1:30)
...
## Задача 4. Локализация: добить остатки (~0:30)
...
- [ ] `grep -rn "initialAccounts\|PremiumInput\|Profile/components/ui/Toast" front/src` → пусто
- [ ] Профиль и вкладка «Безопасность» Настроек показывают одно и то же
      имя без перезагрузки после сохранения
- [ ] Фамилия сохраняется и переживает F5; email не редактируется
- [ ] Переключение студии из профиля приводит в `/dashboard` нужной студии
- [ ] Ни одной русской строки в JSX; обе локали содержат одинаковый набор ключей
