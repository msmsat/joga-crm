...
Статус на 04.09.2026 (обновлено после повторного read-only аудита). Код написан и
покрыт тестами; **в production ничего не изменено, налоговая политика не
подтверждена**. Ручной режим выключен по умолчанию.

> **Состояние Stripe изменилось с прошлой редакции.** Read-only снимок 04.09.2026:
> обе подписки (`sub_1U3O1B…`, `sub_1U3O7H…`) теперь **`canceled`**, оба ранее
> открытых счёта — **`void`**, черновых и открытых счетов **0**, открытых Checkout
> Sessions **0**, Payment Links **0**, ручных Tax Rates **0**, баланс 0.
> **Объектов, способных породить новый платный расчёт Stripe Tax, на аккаунте
> сейчас нет.** Миграция существующих подписок, которая была главным остаточным
> риском, больше не требуется — переносить нечего. Порядок production-перехода и
> ручные шаги владельца: [`LIVE-MANIFEST-TAX.md`](LIVE-MANIFEST-TAX.md).

**Добавлено после первой редакции:**

* `tax_params`/`item_tax_params` в ручном режиме **fail-closed**: путь без
  налогового решения падает (`TaxDecisionMissing`), а не включает платный расчёт;
* ставка приложена **ровно в одном месте**: на позиции счёта либо на подписке,
  но никогда одновременно;
* фоновая миграция живых подписок закрыта отдельным флагом
  `BILLING_TAX_MIGRATE_EXISTING` — смена `BILLING_TAX_MODE` больше не означает
  скрытую production-миграцию;
* фактура за удержанную онлайн-комиссию при нерешённом расхождении нетто/брутто
  **не выпускается** (`BILLING_ONLINE_FEE_TAX_MODEL`), а не пишется в лог;
* тесты на утечку `Customer.tax_exempt` между документами;
* стенд E2E в Stripe test mode: `back/scripts/tax_e2e.py` (на боевом ключе
