...
# Дополнено финальной приёмкой 11.09.2026 (коммит `897780b`, ветка `main`)

## Новые свидетельства

| № | Сценарий | Чем закрыто | Итог |
|---|---|---|---|
| QA-07 | CRM-запись на индивидуальную услугу целиком по HTTP | `tests/test_hybrid_crm_api.py::test_crm_books_moves_and_cancels_an_individual_service_over_http` — availability → quote → confirm → журнал → перенос → отмена, каждый шаг сверен с БД | ✅ автотест (ASGI) |
| QA-17/18 | Перенос: stale-версия, сохранение брони, рост версии | тот же тест: `VERSION_CONFLICT` на чужой версии, тот же `reservation_id`, тот же интервал, `version+1`, журнал сразу отдаёт новую версию | ✅ автотест |
| QA-22 | Клиент диктует цену/длительность/вместимость/студию, чужого мастера, время не из availability, просроченный quote | `test_crm_rejects_client_supplied_terms_and_foreign_scope` — 422/409, и НИ ОДНОЙ брони в базе после отказа | ✅ автотест |
| QA-23 | Роли на новых маршрутах | `test_admin_may_book_but_trainer_may_not` — тренер 403, админ 200 | ✅ автотест |
| QA-24 | Чужая и отменённая бронь при переносе | `test_resource_reschedule.py::test_move_rejects_a_foreign_or_cancelled_reservation` — 404/409, статус не меняется | ✅ автотест |
| QA-26 | Аудит на РЕАЛЬНЫХ легаси-данных | dev-БД, только чтение: подготовленная студия — 0 находок за 0.7 с на 5154 интервалах; легаси-студия — 38 блокирующих находок, exit 1 | ✅ реальные данные |
| — | Индивидуальную услугу нельзя поставить событием | `test_individual_service_cannot_be_scheduled_as_an_event` — 409 на create и на подмену service_id, база не изменилась | ✅ автотест |
| — | Разрез дашборда реально доходит до ответа API | `test_hybrid_analytics.py::test_overview_endpoint_actually_returns_the_booking_mode_breakdown` (поле объявлено с дефолтом `[]` — без теста его пропажа была бы молчаливой) | ✅ автотест |
| — | Горизонт записи: клиенту ограничен, стойке нет | `test_booking_window_limits_the_client_but_not_the_desk` | ✅ автотест |
| — | Групповые механики у индивидуальной записи | `test_hybrid_write_guards.py::test_group_only_features_are_refused_for_an_individual_booking` — кофе отклонён сервером, `coffee` в базе не изменился | ✅ автотест |
| — | Отметка посещения | `test_attendance_cannot_resurrect_a_cancelled_booking_or_skip_payment` — `hold` и `cancelled` отклонены, обычная бронь отмечается, повтор безопасен | ✅ автотест |
| — | Миграция вверх и вниз | `tests/test_hybrid_migration.py` — вызов `upgrade()`/`downgrade()` ревизии в ОТДЕЛЬНОЙ PostgreSQL-схеме; dev-база не участвует | ✅ автотест |

## Что по-прежнему НЕ проверено

Список не сократился и не должен: это ограничения окружения, а не пробелы,
которые можно закрыть текстом.

1. **Браузерный проход** CRM и Mini-app — UI-раннера в репозитории нет.
2. **Safari / вебвью Instagram на живом устройстве** — мобильная раскладка
   закрыта только статической проверкой `miniapp/scroll.check.ts`.
3. **p95 под нагрузкой** по профилю эпика. Измерено другое: задержка
   availability на dev-объёме (медиана 61 мс на 1 день, 67 мс на 31 день) и
   ПОСТОЯНСТВО числа SQL-запросов — работы «на каждый слот» нет.
4. **Массовое включение режима** на боевых студиях — отдельный авторизуемый
   выпуск.
