...
## Задача 5. Фронт: вкладка «Интеграции» (~2:15) — ✅ выполнено

Все 5 файлов из плана + попутная правка API-слоя, которая была необходима,
но в плане явно не названа:

- **`api/settings/settings.types.ts` / `settings.api.ts` — обнаружен и
  заменён «призрачный» контракт.** В кодовой базе уже существовали
  `settingsApi.getIntegrations()`/`updateIntegration()` и тип `Integration`
  вида `{integration_type, is_active, connected_at, config}` — но нигде не
  вызывались (мёртвый код, видимо задел под этот же эпик до того, как
  контракт бэка задачи 2 устоялся). Форма не совпадала с реальным ответом
  `IntegrationStatus` (`{type, connected, connected_at, details,
  capabilities}`). Заменено на реальный контракт; добавлены
  `connectTelegramIntegration/connectWhatsAppIntegration/connectInstagram`
  и весь набор `google/*` (`getGoogleAuthUrl`, `getGoogleCalendars`,
  `updateGoogleCalendar`, `syncGoogleCalendar`, `disconnectGoogleCalendar`).
  `queryKeys.integrations`/`integration(type)` уже были зарезервированы в
  `api/queryKeys.ts` — переиспользованы как есть.
- **`hooks/useIntegrations.ts`** — на `useQuery`/`useMutation` (образец —
  `useSecurity.ts`/`useChannelIntegrations.ts`, других готовых Query-хуков
  под интеграции в проекте не было). Здесь же — эффект возврата из Google
  (`?google=ok|denied|error`, читает/чистит `useSearchParams`, тост,
  инвалидация) и **кросс-инвалидация с Уведомлениями**: `queryKeys.integrations`
  и `queryKeys.notifyIntegrations` смотрят на одни и те же строки
  `StudioIntegration` на бэке (та самая «точка отсчёта» эпика — Интеграции и
  Уведомления рисуют один backend с двух экранов), поэтому подключение/
  отключение с любой из двух страниц инвалидирует оба ключа — иначе один
  экран отставал бы от другого до ручного F5. Правка кросс-инвалидации
  внесена и в существующий `Notifications/hooks/useChannelIntegrations.ts`
  (2 строки) — это единственный файл вне списка задачи 5, тронутый ради
  этой связки.
- **Отключение Google — не через общую ручку.** `disconnect()`
  специально ветвится: `type === "google_calendar"` идёт в
  `disconnectGoogleCalendar()` (задача 4, отзывает токен у Google и обнуляет
  `Lesson.gcal_event_id`), остальные три — в общий `DELETE
  /integrations/{type}` (задача 2, который для google_calendar этого не
  делает вовсе — просто `is_connected=false`). Перепутать эти два пути значило
  бы оставить висящий `refresh_token` у Google и мусорные `gcal_event_id`.
- **`IntegrationsTab` переведён с «поднятого» на самодостаточный хук** —
  раньше `useIntegrations()` вызывался в `Settings.tsx` и пробрасывался
  пропсами (как всё ещё `useBilling`), остальные вкладки
  (Security/Notifications/General/Appearance/Data) уже работали
  самодостаточно. Перевёл `IntegrationsTab` во второй лагерь — так эффект
  возврата из Google живёт рядом с остальной логикой хука, а не размазан
  между `Settings.tsx` и вкладкой.
- **`Settings.tsx`: начальная вкладка теперь читается из `?tab=`.** Без этого
  редирект бэка `.../settings?tab=integrations&google=ok` приземлял бы
  владельца на вкладку «Основные» с потерянным статусом — минимальное,
  но необходимое дополнение (`useState(() => searchParams.get('tab') ??
  'general')`), не полноценный переход всей страницы на URL-роутинг вкладок.
- **`google/start` без ticket-эндпоинта** (см. подробное обоснование и
  отсылку к `routers/ai/instagram.py::get_instagram_oauth_url` в задаче 4) —
  `startGoogleAuth` здесь просто `useMutation` на `getGoogleAuthUrl()` (fetch
  с обычным Authorization-заголовком) → `window.location.assign(url)` в
  `onSuccess`. Ни ticket, ни popup не понадобились.
- **`settings.json` (ru/en) переписан по факту, не только «секция
  integrations».** Так как компоненты этой задачи не могут быть
  осмысленно построены без реального содержимого гайдов/капабилити/ошибок
  Google — весь контент задачи 6 «шаги гайдов (3–5 на интеграцию),
  последствия отключения, ошибки Google» уже здесь: `integrations.guide.*`
  (3–5 шагов на канал), `integrations.capabilities.*` (текст для
  DisconnectModal, ключи — те же строки, что отдаёт `capabilities` бэка:
  `notify`/`booking`/`dm_agent`/`schedule_sync`), `integrations.google.*`
  (`connected`/`denied`/`error`, режимы синхронизации, `syncResult`). Заодно
  удалены ключи `items.onec`/`yandex` и весь старый `fields.onec*/yandex*` —
  задача 1 сознательно оставляла их task 6; вместо «удалить отдельно» они
  просто не попали в новую секцию при переписывании.
- **`constants.ts`/`types.ts` — чистка, которую явно называет задача 6,
  оказалась неотделима от задачи 5:** `INITIAL_INTEGRATIONS_CONFIG` и тип
  `IntegrationsConfig` физически не могли остаться — новый хук вообще не
  сеет стейт из моков, только Query. Убраны как естественное следствие
  переписывания, не отдельным проходом.

**Проверено:** `tsc -b` и `vite build` — зелёные; `npm run lint` — 0 новых
предупреждений/ошибок в изменённых файлах (79 из 79 проблем — в файлах,
которых эта задача не касалась: Journal/Loyalty/Staff/utils, тот же фон, что
и до эпика). `grep -rn "yandex\|onec" front/src` — совпадений по реальному
коду/ключам интеграций больше нет (остались только не относящиеся к делу
`oneClick`/`dropzoneChange`/`alexey@yandex.ru`-мок — были и до эпика).
Живой прогон в браузере не делался (по умолчанию не поднимаем dev-сервер —
см. договорённость сессии); при желании живой проверки скажите явно.
...
## Задача 6. Локализация и очистка (~1:00) — ✅ выполнено (сделано внутри задачи 5)

Компоненты задачи 5 нельзя было осмысленно построить без реального
содержимого гайдов/последствий/ошибок Google, поэтому весь объём этой
задачи — `settings.json` (ru/en, названия/описания/шаги гайдов/последствия
отключения/ошибки Google), `constants.ts` без `INITIAL_INTEGRATIONS_CONFIG`,
`types.ts` без `IntegrationsConfig` — уже сделан и описан в отчёте задачи 5
выше. Отдельного прохода не потребовалось; здесь фиксируется только сам факт
закрытия задачи.
...
## Финальная сверка с «Критериями приёмки EPIC 6»

После задачи 6 прогнан весь список критериев приёмки (ниже) построчно —
нашлось и сразу починено одно реальное расхождение:

- **Колбэк с подделанным `state` → 400 — было не так, теперь так.**
  Изначальная реализация (`google_calendar_callback`) на любой невалидный
  `state` (включая подделанный/протухший/с чужим `purpose`) отвечала мягким
  302-редиректом `?google=error` — так задумано для «обычных» путей отказа
  (п. 4.2 ТЗ: «Ошибка/отказ пользователя → редирект, а не 500»), но
  расходилось с явным критерием приёмки. Починено: `state` **отсутствует**
  (прямой заход на URL колбэка мимо флоу) — по-прежнему мягкий редирект;
  `state` **присутствует, но не прошёл проверку подписи/purpose/TTL** —
  теперь `HTTPException(400)`, без касания БД. Тест
  `test_google_calendar.py` это явно проверяет (400 + интеграция не создаётся).
- Остальные пункты списка (порядок 4 интеграций и отсутствие Яндекс/1С,
  статусы = БД, отсутствие полного токена в ответах, чекбокс-гейт на
  отключении, набор Google-эндпоинтов, отзыв доступа → sync без 500,
  build+lint) — подтверждены кодом/тестами по ходу задач 1–6, отдельных
  расхождений не найдено.
- Заодно закрыт edge-case «часовой пояс студии не задан», который был описан
  в ТЗ, но не был реализован ни в одной из задач 1–6: `IntegrationCard`
  теперь показывает предупреждение под настройками Google, если
  `Studio.timezone` пуст (запрос — тот же `queryKeys.studioSettings`, что
  уже использует `useGeneralSettings`/`useStudioCurrency`, обычно уже в
  кэше — лишнего запроса на большинстве экранов не добавляет).
- **Не проверено и не может быть проверено без реального Google-приложения**
  (см. также задачи 3/4): полный E2E `consent screen → «Подключено» →
  занятие появляется в календаре`, а также «отзыв доступа на стороне Google
  → следующий sync ловит 401 и не роняет 500» — обработка 401 реализована и
  прочитана построчно (`push_lesson`/`sync_studio` перехватывают
  `GcalAuthError`, не пробрасывают исключение), но не исполнялась против
  настоящего отозванного токена.
